[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_86_clare_risk_parity_trend\evaluation"
)
...
foreach ($Name in $UnsafeNames) {
    if (-not [string]::IsNullOrEmpty([System.Environment]::GetEnvironmentVariable($Name, "Process"))) {
        [Console]::Error.WriteLine("clare_risk_parity_trend_tournament_status=blocked_unsafe_environment")
        exit 2
    }
}
...
try {
    ...
    & python -m algotrader.research.clare_risk_parity_trend_tournament --output-root $OutputRoot
    exit $LASTEXITCODE
}
finally {
