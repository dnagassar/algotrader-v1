<#
.SYNOPSIS
Acquires the seventeen V5.97 Tier A cohort 2 ETF histories.
...
param(
    ...
    [string]$OutputRoot = "runs\v5_97_tier_a_cohort_2",
...
try {
    ...
    if ($Mode -eq "dry_run") {
        Write-Output "v597_cohort2_data_refresh_status=dry_run_complete"
    }
    else {
        Write-Output "v597_cohort2_data_refresh_status=completed"
    }
    exit 0
}
finally {
