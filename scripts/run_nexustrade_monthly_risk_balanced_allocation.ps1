<#
.SYNOPSIS
Runs the preregistered offline V5.67 risk-balanced allocation replay.

.DESCRIPTION
Validates the committed V5.67 protocol, frozen V5.64 dependencies and
...
[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_67_nexustrade_monthly_risk_balanced_allocation",
    ...
    [string]$PreregistrationPath = "docs\design\v5_67_nexustrade_monthly_risk_balanced_allocation.md",
...
if ($AppProfile -in @("paper", "live") -or $SensitiveLoaded) {
    [Console]::Error.WriteLine(
        "nexustrade_monthly_risk_balanced_status=blocked_unsafe_environment"
    )
    exit 2
}
...
try {
    ...
    & python -m algotrader.research.nexustrade_monthly_risk_balanced_allocation `
