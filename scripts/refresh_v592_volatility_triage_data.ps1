<#
.SYNOPSIS
Acquires the seventeen V5.92 volatility triage ETF histories.
...
param(
    ...
    [string]$DataStart = "2000-01-03",
    [string]$ExpectedLatestBarDate = "2026-07-31",
    [string]$OutputRoot = "runs\v5_92_vault_volatility_managed_triage",
...
try {
    ...
    if ($Mode -eq "dry_run") {
        Write-Output "v592_volatility_triage_data_refresh_status=dry_run_complete"
    }
    else {
        Write-Output "v592_volatility_triage_data_refresh_status=completed"
    }
    exit 0
}
finally {
