<#
.SYNOPSIS
Acquires the seventeen V5.93 reversal triage ETF histories.
...
param(
    ...
    [string]$OutputRoot = "runs\v5_93_nonequity_reversal_triage",
...
try {
    ...
    if ($Mode -eq "dry_run") {
        Write-Output "v593_reversal_triage_data_refresh_status=dry_run_complete"
    }
    else {
        Write-Output "v593_reversal_triage_data_refresh_status=completed"
    }
    exit 0
}
finally {
