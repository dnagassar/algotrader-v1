[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_89_keller_bold_asset_allocation\evaluation"
)
...
foreach ($Name in $UnsafeNames) {
    if (-not [string]::IsNullOrEmpty([System.Environment]::GetEnvironmentVariable($Name, "Process"))) {
        [Console]::Error.WriteLine("baa_tournament_status=blocked_unsafe_environment")
        exit 2
    }
}
...
try {
    ...
    & python -m algotrader.research.baa_tournament --output-root $OutputRoot
    exit $LASTEXITCODE
}
finally {
