[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_87_keller_flexible_asset_allocation\evaluation"
)
...
foreach ($Name in $UnsafeNames) {
    if (-not [string]::IsNullOrEmpty([System.Environment]::GetEnvironmentVariable($Name, "Process"))) {
        [Console]::Error.WriteLine("keller_flexible_asset_allocation_tournament_status=blocked_unsafe_environment")
        exit 2
    }
}
...
try {
    ...
    & python -m algotrader.research.keller_flexible_asset_allocation_tournament --output-root $OutputRoot
    exit $LASTEXITCODE
}
finally {
