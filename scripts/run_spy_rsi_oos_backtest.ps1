<#
.SYNOPSIS
Runs the deterministic fixed-parameter SPY RSI OOS backtest gate.

.DESCRIPTION
Evaluates the SPY RSI-14 mean-reversion shadow candidate against buy-and-hold
SPY and the SMA50/200 training-wheel baseline using local adjusted daily bars
only. This wrapper does not read a broker, mutate broker state, submit paper
orders, load credentials, or contact the network. Credential values are never
printed.

.PARAMETER OutputRoot
Directory under which ignored RSI OOS artifacts are written.
...
[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\strategy_challengers\rsi_oos\latest",
    [string]$BarsCsv = "runs\operator_input\m446_spy_daily_tiingo_adjusted_canonical.csv",
    [string]$AsOfDate,
    [string]$RunId = "v3_8_spy_rsi_fixed_parameter_oos_backtest",
...
if ($AppProfileIsPaper -or $AppProfileIsLive -or $CredentialVariablesLoaded) {
    Write-Host "spy_rsi_oos_backtest_status=blocked_unsafe_environment"
    exit 2
}

$Python = Get-PythonCommand
$Args = @(
    "-m", "algotrader.research.spy_rsi_oos_backtest",
