<#
.SYNOPSIS
Acquires the seventeen V5.89 Bold Asset Allocation ETF histories.

.DESCRIPTION
Runs seventeen sequential allowlisted Tiingo EOD GETs through the existing
...
param(
    ...
    [string]$OutputRoot = "runs\v5_89_keller_bold_asset_allocation",
...
try {
    ...
    if ($Mode -eq "dry_run") {
        Write-Output "v589_baa_data_refresh_status=dry_run_complete"
    }
    else {
        Write-Output "v589_baa_data_refresh_status=completed"
    }
    exit 0
}
finally {
