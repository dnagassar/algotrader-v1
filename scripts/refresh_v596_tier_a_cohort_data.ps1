<#
.SYNOPSIS
Acquires the seventeen V5.96 Tier A cohort ETF histories.

.DESCRIPTION
Runs eighteen sequential allowlisted Tiingo EOD GETs through the existing
...
param(
    ...
    [string]$DataStart = "2005-01-03",
    [string]$ExpectedLatestBarDate = "2026-07-31",
    [string]$OutputRoot = "runs\v5_96_tier_a_component_cohort",
...
$Symbols = @(
    "VGLT", "EDV", "GOVT", "USMV", "SPHD", "NOBL", "VCSH",
    "BKLN", "FLOT", "MTUM", "SMH", "VBK", "SPY"
)
...
try {
    ...
    foreach ($Symbol in $Symbols) {
        ...
        if ($LASTEXITCODE -ne 0) {
            [Console]::Error.WriteLine("Error: V5.89 adjusted data refresh failed for $Symbol.")
            exit $LASTEXITCODE
        }
    }
    if ($Mode -eq "dry_run") {
        Write-Output "v596_tier_a_cohort_data_refresh_status=dry_run_complete"
    }
    else {
        Write-Output "v596_tier_a_cohort_data_refresh_status=completed"
    }
    exit 0
}
finally {
