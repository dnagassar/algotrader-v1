<#
.SYNOPSIS
Acquires the seventeen V5.91 vault triage ETF histories.
...
param(
    ...
    [string]$DataStart = "1996-04-01",
    [string]$ExpectedLatestBarDate = "2026-07-31",
    [string]$OutputRoot = "runs\v5_91_vault_cross_sectional_trend_triage",
...
try {
    ...
    if ($Mode -eq "dry_run") {
        Write-Output "v591_vault_triage_data_refresh_status=dry_run_complete"
    }
    else {
        Write-Output "v591_vault_triage_data_refresh_status=completed"
    }
    exit 0
}
finally {
