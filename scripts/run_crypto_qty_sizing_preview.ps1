...
.DESCRIPTION
Consumes local router and crypto refresh artifacts, builds a bounded paper-lab
quantity preview, and writes ignored operating artifacts. This wrapper does not
...
param(
    ...
    [string]$OpportunityCandidates = "runs\opportunity_router\paper_read_repair_latest\opportunity_candidates.json",
    [string]$CryptoRouterInputManifest = "runs\crypto_universe_refresh\paper_read_repair_latest\crypto_router_input_manifest.json",
...
Write-Host "crypto_qty_sizing_preview_command=run_crypto_qty_sizing_preview"
Write-Host "crypto_qty_sizing_preview_mode=offline/no_submit"
Write-Host "crypto_qty_sizing_preview_no_submit_enforced=true"
...
Write-Host "crypto_qty_sizing_preview_paper_submit_authorized=false"
Write-Host "crypto_qty_sizing_preview_paper_submit_performed=false"
Write-Host "crypto_qty_sizing_preview_broker_mutation_performed=false"
Write-Host "crypto_qty_sizing_preview_live_mutation_performed=false"

if ($AppProfileIsPaper -or $AppProfileIsLive -or $CredentialVariablesLoaded -or $NetworkFlagsLoaded -or $LiveEndpointIndicator) {
    Write-Host "crypto_qty_sizing_preview_status=blocked_unsafe_environment"
    exit 2
}

$Python = Get-PythonCommand
$Args = @(
    "-m", "algotrader.orchestration.crypto_qty_sizing_preview",
    "--output-root", $OutputRoot,
    "--router-decision", $RouterDecision,
    "--opportunity-candidates", $OpportunityCandidates,
    "--crypto-router-input-manifest", $CryptoRouterInputManifest,
