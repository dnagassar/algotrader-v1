[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_70_v564_frozen_forward_confirmation",
...
if ($AppProfile -in @("paper", "live") -or $SensitiveLoaded) {
    [Console]::Error.WriteLine("v564_frozen_forward_status=blocked_unsafe_environment")
    exit 2
}
...
try {
    ...
    & python -m algotrader.research.v564_frozen_forward_confirmation `
