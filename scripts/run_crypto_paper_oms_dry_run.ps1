...
.DESCRIPTION
Consumes a local v5.5 crypto Paper OMS handoff packet, builds a deterministic
broker-neutral dry-run record, and writes ignored operating artifacts. This
...
Write-Host "crypto_paper_oms_dry_run_command=run_crypto_paper_oms_dry_run"
Write-Host "crypto_paper_oms_dry_run_mode=offline/no_submit"
Write-Host "crypto_paper_oms_dry_run_no_submit_enforced=true"
...
Write-Host "crypto_paper_oms_dry_run_paper_submit_authorized=false"
Write-Host "crypto_paper_oms_dry_run_paper_submit_performed=false"
Write-Host "crypto_paper_oms_dry_run_broker_mutation_performed=false"
Write-Host "crypto_paper_oms_dry_run_live_mutation_performed=false"

if ($AppProfileIsPaper -or $AppProfileIsLive -or $CredentialVariablesLoaded -or $NetworkFlagsLoaded -or $LiveEndpointIndicator) {
    Write-Host "crypto_paper_oms_dry_run_status=blocked_unsafe_environment"
    exit 2
}

$Python = Get-PythonCommand
$Args = @(
    "-m", "algotrader.orchestration.crypto_paper_oms_dry_run",
