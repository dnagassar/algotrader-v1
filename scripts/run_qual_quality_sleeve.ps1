[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_78_qual_quality_sleeve\evaluation",
...
if ($Unsafe) {
    [Console]::Error.WriteLine("qual_quality_sleeve_status=blocked_unsafe_environment")
    exit 2
}
...
try {
    ...
    & python -m algotrader.research.qual_quality_sleeve --output-root $OutputRoot --format $Format
    exit $LASTEXITCODE
}
finally {
    [System.Environment]::SetEnvironmentVariable(
