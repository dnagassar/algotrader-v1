<#
.SYNOPSIS
Runs the preregistered offline V5.66 high-volatility attribution diagnostic.

.DESCRIPTION
Validates the committed V5.66 protocol, frozen V5.64/V5.65 dependencies, and
canonical V5.63 data hashes before replay. The command is credential-free,
network-free, broker-free, no-submit, and creates no candidate or route.
#>

[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_66_nexustrade_high_volatility_attribution",
    ...
    [string]$PreregistrationPath = "docs\design\v5_66_nexustrade_high_volatility_attribution.md",
...
if ($AppProfile -in @("paper", "live") -or $SensitiveLoaded) {
    [Console]::Error.WriteLine(
        "nexustrade_high_volatility_attribution_status=blocked_unsafe_environment"
    )
    exit 2
}
...
try {
    ...
    & python -m algotrader.research.nexustrade_high_volatility_attribution `
