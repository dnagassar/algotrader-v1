<#
.SYNOPSIS
Runs the preregistered offline V5.65 high-volatility defense.

.DESCRIPTION
Validates the committed V5.65 protocol, frozen V5.64 dependencies, and
canonical V5.63 data hashes before replay. The command is credential-free,
network-free, broker-free, no-submit, and not an authentic replay of the March
2025 NexusTrade historical run.
#>

[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_65_nexustrade_monthly_high_volatility_defense",
    ...
    [string]$PreregistrationPath = "docs\design\v5_65_nexustrade_monthly_high_volatility_defense.md",
...
if ($AppProfile -in @("paper", "live") -or $SensitiveLoaded) {
    [Console]::Error.WriteLine(
        "nexustrade_monthly_high_volatility_defense_status=blocked_unsafe_environment"
    )
    exit 2
}
...
try {
    ...
    & python -m algotrader.research.nexustrade_monthly_high_volatility_defense `
