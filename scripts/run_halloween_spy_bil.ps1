[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_76_halloween_spy_bil\evaluation",
...
if ($Unsafe) {
    [Console]::Error.WriteLine("halloween_spy_bil_status=blocked_unsafe_environment")
    exit 2
}
...
try {
    ...
    & python -m algotrader.research.halloween_spy_bil --output-root $OutputRoot --format $Format
    exit $LASTEXITCODE
}
finally {
    [System.Environment]::SetEnvironmentVariable(
