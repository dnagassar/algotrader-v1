[CmdletBinding()]
param(
    [string]$OutputRoot = "runs\v5_84_factor_momentum_style_proxy\evaluation"
)
...
foreach ($Name in $UnsafeNames) {
    if (-not [string]::IsNullOrEmpty([System.Environment]::GetEnvironmentVariable($Name, "Process"))) {
        [Console]::Error.WriteLine("factor_momentum_style_tournament_status=blocked_unsafe_environment")
        exit 2
    }
}
...
try {
    ...
    & python -m algotrader.research.factor_momentum_style_tournament --output-root $OutputRoot
    exit $LASTEXITCODE
}
finally {
