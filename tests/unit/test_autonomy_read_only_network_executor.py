...
def test_allowlist_routes_the_integrated_spy_cycle() -> None:
    ...
    assert "autonomy_spy_refresh_cycle" in cmd
    assert "--as-of" in cmd
    assert "[--apply]" in cmd
...
def test_successful_apply_writes_reservation_and_completion_events(
    ...
    def _dummy_run_spy_refresh(
        config: Any,
        token_lookup: Any,
        http_get: Any,
    ) -> dict[str, Any]:
        assert token_lookup("TIINGO_API_KEY") == "test_token_value"
        assert http_get is executor.tiingo_bounded_http_get
        return {"refresh_state": "accepted_adjusted_spy_data_refresh"}
    ...
    assert result["exit_code"] == 0
    assert result["adapter_refresh_state"] == "accepted_adjusted_spy_data_refresh"
    ...
    assert completed_event["adapter_refresh_state"] == (
        "accepted_adjusted_spy_data_refresh"
    )
...
def test_seam_freezes_the_read_only_tiingo_refresh_configuration(
    ...
    assert captured["http_get"] is executor.tiingo_bounded_http_get
