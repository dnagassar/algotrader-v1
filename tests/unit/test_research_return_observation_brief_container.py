...
MODULE_PATH = Path(
    "src/algotrader/research/research_return_observation_brief_container.py"
)
...
_BRIEF_ID = "research-return-observation-brief:synthetic:broad-etf-return"
_TITLE = "Synthetic broad ETF return observation brief"
_SUMMARY = "Research return observation brief contains 1 advisory-only section."
_ALLOWED_IMPORTS = {
    "__future__",
    "collections.abc",
    "dataclasses",
...
def test_builds_brief_from_phase_215_synthetic_section_fixture() -> None:
...
def test_preserves_exact_section_identity_and_order() -> None:
    ...
    assert brief.to_dict()["sections"] == [first.to_dict(), second.to_dict()]
...
def test_rejects_non_sections_malformed_lookalikes_and_subclasses() -> None:
    class Lookalike:
        def to_dict(self) -> dict[str, object]:
            return expected_synthetic_research_return_observation_brief_section_dict()

    class SectionSubclass(ResearchReturnObservationBriefSection):
        pass

    source = build_synthetic_research_return_observation_brief_section()
...
def test_requires_non_empty_brief_id_title_and_summary() -> None:
    ...
    for brief_id, title, summary in (
        ...
        ("  bad", _TITLE, _SUMMARY),
        (_BRIEF_ID, "", _SUMMARY),
        (_BRIEF_ID, _join("author", "ity title"), _SUMMARY),
        (_BRIEF_ID, _TITLE, ""),
        (_BRIEF_ID, _TITLE, _join("action", "ability summary")),
    ):
...
def test_fixed_advisory_metadata_is_pinned() -> None:
    ...
    assert payload["brief_type"] == "research_return_observation_brief"
...
def test_direct_construction_rejects_metadata_mismatches() -> None:
    ...
    for field_name, value in (
        ...
        with pytest.raises(ValidationError):
            ResearchReturnObservationBrief(**mutated)
...
def test_rejects_authority_and_actionability_wording_outside_non_claims() -> None:
    ...
    for field_name, value in (
        ("brief_id", _join("author", "ity-brief")),
...
def test_source_section_to_dict_output_is_unchanged() -> None:
    ...
    assert before == expected_synthetic_research_return_observation_brief_section_dict()
...
def test_object_is_frozen_and_slotted() -> None:
    ...
    assert hasattr(ResearchReturnObservationBrief, "__slots__")
    assert tuple(field.name for field in fields(ResearchReturnObservationBrief)) == (
        "brief_type",
        "status",
        "authority",
        "capital_authority",
...
def _build_default_brief() -> ResearchReturnObservationBrief:
    return _build_brief(build_synthetic_research_return_observation_brief_section())


def _build_brief(
    section: ResearchReturnObservationBriefSection,
) -> ResearchReturnObservationBrief:
