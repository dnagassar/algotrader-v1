...
from algotrader.research import (
    research_return_observation_brief_export as export_module,
)
from algotrader.research.research_return_observation_brief_container import (
    ResearchReturnObservationBrief,
)
from algotrader.research.research_return_observation_brief_export import (
...
_EXPECTED_PAYLOAD = expected_synthetic_research_return_observation_brief_dict()
_EXPECTED_JSON_TEXT = json.dumps(
    _EXPECTED_PAYLOAD,
    sort_keys=True,
    separators=(",", ":"),
)
_EXPECTED_RENDERED_TEXT = render_research_return_observation_brief_text(
    build_synthetic_research_return_observation_brief()
)
...
def test_export_builder_accepts_phase_217_fixture_and_matches_views() -> None:
    ...
    exported = export_research_return_observation_brief(brief)

    assert type(brief) is ResearchReturnObservationBrief
    assert type(exported) is ResearchReturnObservationBriefExport
    ...
    assert exported.rendered_text == render_research_return_observation_brief_text(
        brief
    )
...
def test_json_text_is_compact_deterministic_and_round_trips_to_payload() -> None:
    exported = export_research_return_observation_brief(
        build_synthetic_research_return_observation_brief()
    )
...
def test_rendered_text_matches_phase_218_renderer_output() -> None:
    brief = build_synthetic_research_return_observation_brief()
    ...
    assert exported.rendered_text.startswith("Research Return Observation Brief")
...
def test_repeated_exports_are_byte_for_byte_deterministic() -> None:
    ...
    first = export_research_return_observation_brief(brief)
    second = export_research_return_observation_brief(brief)
    third = export_research_return_observation_brief(
        build_synthetic_research_return_observation_brief()
    )
...
def test_source_brief_to_dict_is_unchanged_before_and_after_export() -> None:
    ...
    export_research_return_observation_brief(brief)
    export_research_return_observation_brief(brief)

    assert brief.to_dict() == before == _EXPECTED_PAYLOAD


def test_section_item_source_observation_and_return_point_identities_stay_stable() -> None:
    brief = build_synthetic_research_return_observation_brief()
...
def test_direct_construction_validates_copies_and_is_frozen_slotted() -> None:
    ...
    exported = ResearchReturnObservationBriefExport(
        payload=payload,
    ...
    assert tuple(field.name for field in fields(ResearchReturnObservationBriefExport)) == (
        _EXPECTED_FIELD_NAMES
    )
    assert hasattr(ResearchReturnObservationBriefExport, "__slots__")
...
def test_exact_brief_type_validation_rejects_non_briefs_and_lookalikes() -> None:
    for value in (
        ...
        with pytest.raises(ValidationError, match="ResearchReturnObservationBrief"):
            export_research_return_observation_brief(value)


def test_exact_brief_type_validation_rejects_subclasses() -> None:
    brief = _brief_subclass(build_synthetic_research_return_observation_brief())
...
def test_no_from_dict_exists() -> None:
    ...
    assert not hasattr(ResearchReturnObservationBriefExport, "from_dict")
...
def _brief_subclass(
    source: ResearchReturnObservationBrief,
) -> BriefSubclass:
...
def _change_payload_copy(payload: dict[str, object]) -> None:
    ...
    returns = _list(observation["returns"])
    ...
    _dict(returns[0])["simple_return"] = "changed primitive return payload copy"
...
def _export_ast_fields() -> set[str]:
    for node in ast.walk(_tree()):
        if (
            ...
            and node.name == "ResearchReturnObservationBriefExport"
        ):
            return {
                statement.target.id
    ...
    raise AssertionError("ResearchReturnObservationBriefExport was not found.")
...
def _export_return_keyword_names() -> set[str]:
    ...
    for node in ast.walk(_tree()):
        if (
            ...
            and _call_name(node.func) == "ResearchReturnObservationBriefExport"
        ):
