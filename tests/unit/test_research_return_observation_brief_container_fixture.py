...
MODULE_PATH = Path("tests/fixtures/research_return_observation_brief_container.py")
...
_ALLOWED_IMPORTS = {
    ...
    "tests.fixtures.research_return_observation_brief_section",
}
...
def test_fixture_builder_returns_exact_phase_216_production_type() -> None:
...
def test_expected_dict_helper_matches_to_dict_exactly() -> None:
    ...
    assert brief.to_dict() == expected_synthetic_research_return_observation_brief_dict()


def test_expected_dict_helper_returns_fresh_mutable_primitive_copies() -> None:
    first = expected_synthetic_research_return_observation_brief_dict()
    second = expected_synthetic_research_return_observation_brief_dict()
    ...
    assert first["sections"][0]["items"][1] is not second["sections"][0]["items"][1]
    assert first["sections"][0]["items"][0]["source_observation"] is not (
        second["sections"][0]["items"][0]["source_observation"]
    )
    assert first["sections"][0]["items"][0]["source_observation"][
        "returns"
    ] is not (
        second["sections"][0]["items"][0]["source_observation"]["returns"]
    )
    assert first["sections"][0]["limitations"] is not (
        second["sections"][0]["limitations"]
    )
    assert first["sections"][0]["non_claims"] is not (
        second["sections"][0]["non_claims"]
    )
    assert first["limitations"] is not second["limitations"]
    assert first["non_claims"] is not second["non_claims"]

    first["sections"][0]["items"][0]["source_observation"]["returns"][0][
        "simple_return"
    ] = "mutated"
    first["sections"][0]["items"][1]["source_observation"]["returns"].append(
        {"mutated": "copy"}
    )
    first["sections"][0]["limitations"].append("mutated primitive copy")
    first["sections"][0]["non_claims"].append("not mutated primitive copy")
    first["limitations"].append("mutated primitive copy")
    first["non_claims"].append("not mutated primitive copy")

    assert second == build_synthetic_research_return_observation_brief().to_dict()


def test_repeated_construction_is_deterministic() -> None:
    first = build_synthetic_research_return_observation_brief()
    second = build_synthetic_research_return_observation_brief()
...
def test_compact_json_bytes_are_deterministic() -> None:
    first = _compact_json_bytes(build_synthetic_research_return_observation_brief())
    second = _compact_json_bytes(build_synthetic_research_return_observation_brief())
...
def test_brief_contains_exactly_one_section_in_expected_order() -> None:
    ...
    assert brief.brief_type == "research_return_observation_brief"
...
def test_nested_section_payload_matches_phase_215_expected_section_dict() -> None:
    brief = build_synthetic_research_return_observation_brief()
...
def test_section_object_identity_is_preserved() -> None:
    ...
    brief = build_research_return_observation_brief(
        brief_id=_BRIEF_ID,
        title=_TITLE,
        summary=_SUMMARY,
        sections=(section,),
...
def test_nested_primary_item_retains_return_count_metadata() -> None:
    brief = build_synthetic_research_return_observation_brief()
    primary_item = brief.sections[0].items[0]
    primary_payload = brief.to_dict()["sections"][0]["items"][0]
...
def test_nested_insufficient_history_item_retains_state() -> None:
    brief = build_synthetic_research_return_observation_brief()
    insufficient_item = brief.sections[0].items[1]
    insufficient_payload = brief.to_dict()["sections"][0]["items"][1]

    assert insufficient_item.mechanical_state == "insufficient_return_history"
    assert insufficient_payload["mechanical_state"] == "insufficient_return_history"
...
def test_limitations_and_non_claims_carry_forward() -> None:
    ...
    assert brief.to_dict()["limitations"] == (
        expected_synthetic_research_return_observation_brief_dict()[
            "limitations"
        ]
    )
    assert brief.to_dict()["non_claims"] == (
        expected_synthetic_research_return_observation_brief_dict()[
            "non_claims"
        ]
    )
...
def test_no_from_dict_exists() -> None:
    ...
    assert not hasattr(ResearchReturnObservationBrief, "from_dict")
...
def test_no_positive_action_or_authority_states_appear() -> None:
    payload = build_synthetic_research_return_observation_brief().to_dict()
...
def _compact_json_bytes(brief: ResearchReturnObservationBrief) -> bytes:
    return json.dumps(
        brief.to_dict(),
        sort_keys=True,
        separators=(",", ":"),
