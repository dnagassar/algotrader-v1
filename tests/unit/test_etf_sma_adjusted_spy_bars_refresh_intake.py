...
@pytest.mark.parametrize(
    "symbol",
    ("QQQ", "XLK", "VEU", "VEA", "LQD", "EFA", "VNQ", "DBC", "QUAL", "PBUS", "IWD", "IWF", "RSP", "VBR", "VIG", "SPLV", "URTH", "VTI", "GSG", "EEM", "EWJ", "ICF", "RWX", "VGK", "HYG", "TIP", "EWA", "EWZ", "INDA", "TUR", "USO", "FXE", "MTUM", "VGLT", "GDX", "VLUE", "BRK-B"),
)
def test_approved_non_spy_symbol_can_be_validated_offline(
    tmp_path,
    symbol: str,
) -> None:  # noqa: ANN001
    input_csv = tmp_path / f"{symbol.lower()}.csv"
    rows = [
        "symbol,date,open,high,low,close,adjusted_close,volume",
        f"{symbol},2026-06-06,100,101,99,100,100.25,1000",
        f"{symbol},2026-06-07,101,102,100,101,101.25,2000",
    ]
    input_csv.write_text("\n".join(rows) + "\n", encoding="utf-8")
    canonical_csv = tmp_path / "canonical.csv"
    run_log = tmp_path / "manifest.jsonl"
    ...
    assert out_rows[1].startswith(f"{symbol},2026-06-06,")
    _assert_safety_false(payload)
