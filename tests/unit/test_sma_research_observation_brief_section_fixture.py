...
MODULE_PATH = Path("tests/fixtures/sma_research_observation_brief_section.py")
...
_ALLOWED_IMPORTS = {
    ...
    "tests.fixtures.sma_research_observation_brief",
}
...
def test_fixture_builder_returns_exact_phase_199_production_type() -> None:
    section = build_synthetic_sma_research_observation_brief_section()

    assert type(section) is SmaResearchObservationBriefSection
    assert all(type(item) is SmaResearchObservationBriefItem for item in section.items)
...
def test_expected_dict_helper_returns_fresh_mutable_primitive_copies() -> None:
    first = expected_synthetic_sma_research_observation_brief_section_dict()
    second = expected_synthetic_sma_research_observation_brief_section_dict()
    ...
    first["items"][0]["source_observation"]["limitations"].append(
        "mutated nested copy"
    )
    first["limitations"].append("mutated primitive copy")
    first["non_claims"].append("not mutated primitive copy")

    assert second == build_synthetic_sma_research_observation_brief_section().to_dict()
...
def test_compact_json_bytes_are_deterministic() -> None:
    first = _compact_json_bytes(build_synthetic_sma_research_observation_brief_section())
...
def test_nested_item_payloads_match_phase_198_expected_dicts() -> None:
    section = build_synthetic_sma_research_observation_brief_section()
...
def test_item_object_identity_is_preserved() -> None:
    section = build_synthetic_sma_research_observation_brief_section()
    rebuilt = build_sma_research_observation_brief_section(
...
def test_limitations_and_non_claims_carry_forward() -> None:
    ...
    assert section.limitations == first_item.limitations
    assert section.non_claims == first_item.non_claims
    assert section.to_dict()["limitations"] == list(first_item.limitations)
    assert section.to_dict()["non_claims"] == list(first_item.non_claims)
...
def test_no_positive_action_or_authority_states_appear() -> None:
    payload = build_synthetic_sma_research_observation_brief_section().to_dict()
