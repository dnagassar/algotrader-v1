...
MODULE_PATH = Path("src/algotrader/research/sma_research_observation_brief_section.py")
...
_SECTION_ID = "sma-observation-brief-section:synthetic:2026-01-20"
_TITLE = "Synthetic SMA observation metadata group"
_SUMMARY = (
    "SMA observation metadata group contains 2 candidate item(s): "
    "above_sma_observation and insufficient_history."
)
_ALLOWED_IMPORTS = {
    "__future__",
    "collections.abc",
...
def test_builds_section_from_phase_198_fixtures() -> None:
    above_item = build_synthetic_sma_research_observation_brief_item()
    insufficient_item = (
        build_synthetic_insufficient_history_sma_research_observation_brief_item()
    )
    section = _build_section(above_item, insufficient_item)

    assert type(section) is SmaResearchObservationBriefSection
    assert section.items == (above_item, insufficient_item)
...
def test_preserves_exact_item_identity_and_order() -> None:
    ...
    assert section.items[0] is above_item
    assert section.items[1] is insufficient_item
...
def test_rejects_non_items_lookalikes_and_subclasses() -> None:
    class Lookalike:
        def to_dict(self) -> dict[str, object]:
            return expected_synthetic_sma_research_observation_brief_item_dict()

    class ItemSubclass(SmaResearchObservationBriefItem):
        pass

    source = build_synthetic_sma_research_observation_brief_item()
...
def test_requires_non_empty_section_id_title_and_summary() -> None:
    ...
    for section_id, title, summary in (
        ...
        (_SECTION_ID, _join("app", "roved title"), _SUMMARY),
        (_SECTION_ID, _TITLE, ""),
        (_SECTION_ID, _TITLE, _join("b", "uy summary")),
    ):
...
def test_fixed_advisory_metadata_is_pinned() -> None:
    ...
    assert payload["section_type"] == "sma_research_observation_brief_section"
...
def test_limitations_and_non_claims_carry_forward_with_first_seen_dedupe() -> None:
    first = build_synthetic_sma_research_observation_brief_item()
    second = build_synthetic_insufficient_history_sma_research_observation_brief_item()
    section = _build_section(first, second)

    assert section.limitations == first.limitations
    assert section.non_claims == first.non_claims
    assert section.to_dict()["limitations"] == list(first.limitations)
    assert section.to_dict()["non_claims"] == list(first.non_claims)
...
def test_direct_construction_rejects_metadata_mismatches() -> None:
    ...
    for field_name, value in (
        ...
        with pytest.raises(ValidationError):
            SmaResearchObservationBriefSection(**mutated)
...
def test_source_item_to_dict_outputs_are_unchanged() -> None:
    ...
    before = (above_item.to_dict(), insufficient_item.to_dict())
    section = _build_section(above_item, insufficient_item)
    after_build = (above_item.to_dict(), insufficient_item.to_dict())
    payload = section.to_dict()
    after_serialize = (above_item.to_dict(), insufficient_item.to_dict())
...
def test_object_is_frozen_and_slotted() -> None:
    ...
    assert hasattr(SmaResearchObservationBriefSection, "__slots__")
    assert tuple(field.name for field in fields(SmaResearchObservationBriefSection)) == (
        "section_type",
        "status",
        "authority",
        "capital_authority",
...
def test_no_from_dict_exists() -> None:
    ...
    assert not hasattr(SmaResearchObservationBriefSection, "from_dict")
...
def _build_default_section() -> SmaResearchObservationBriefSection:
...
def _build_section(
    first: SmaResearchObservationBriefItem,
    second: SmaResearchObservationBriefItem,
) -> SmaResearchObservationBriefSection:
...
def _expected_section_dict() -> dict[str, object]:
    first = expected_synthetic_sma_research_observation_brief_item_dict()
    second = (
        expected_synthetic_insufficient_history_sma_research_observation_brief_item_dict()
    )
    return {
...
def _compact_json_bytes(section: SmaResearchObservationBriefSection) -> bytes:
    return json.dumps(
        section.to_dict(),
        sort_keys=True,
        separators=(",", ":"),
