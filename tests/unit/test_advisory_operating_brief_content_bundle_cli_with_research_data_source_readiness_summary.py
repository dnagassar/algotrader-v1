...
_SUMMARY_FLAG = "--include-research-data-source-readiness-summary"
...
def test_readiness_summary_preview_flag_is_hidden_boolean_and_non_input_bearing(
    ...
    json_args = parser.parse_args((_SUMMARY_FLAG, "--format", "json"))
    action = parser._option_string_actions[_SUMMARY_FLAG]

    assert _SUMMARY_FLAG not in help_text
    assert _SUMMARY_FLAG not in _option_text(parser)
    assert default_args.include_research_data_source_readiness_summary is False
    assert args.include_research_data_source_readiness_summary is True
    assert args.output_format == "text"
    assert json_args.include_research_data_source_readiness_summary is True
    ...
    for argv in (
        (_COMMAND, f"{_SUMMARY_FLAG}=true"),
        (_COMMAND, _SUMMARY_FLAG, "true"),
    ):
...
def test_readiness_summary_preview_builder_matches_existing_fixture_branch() -> None:
    bundle = (
        build_synthetic_advisory_operating_brief_content_bundle_with_research_data_source_readiness_summary()
    )
    expected = (
    ...
    assert bundle.research_data_source_readiness == ()
    assert bundle.research_data_source_readiness_summaries[0].to_dict() == (
        summary_payload
    )
...
def test_existing_default_text_and_json_output_remain_unchanged(capsys) -> None:
    ...
    assert "Research Data Source Readiness Summary Diagnostics" not in text_output
    assert "research_data_source_readiness_summaries" not in json_output


def test_include_readiness_summary_text_includes_counts_and_limitations(
    capsys,
) -> None:
    ...
    assert "Research Data Source Readiness Summary Diagnostics" in output
    assert "research_data_source_readiness_summary_count: 1" in output
    assert "summary_state: candidate_only" in output
    assert "required_control_count: 6" in output
    assert "satisfied_control_count: 1" in output
    assert "missing_control_count: 5" in output
    assert "diagnostic_limitations:" in output
    for limitation in summary_payload["diagnostic_limitations"]:
        assert f"- {limitation}" in output
    assert "Research Data Source Readiness Diagnostics" not in output
    assert "source_readiness:" not in output
    assert "missing_controls:" not in output


def test_include_readiness_summary_json_is_compact_and_round_trips(capsys) -> None:
    expected = _expected_summary_export()
    summary_payload = expected_synthetic_research_data_source_readiness_summary_dict(
        build_research_data_source_readiness_summary
    )
    ...
    branch = payload["research_data_source_readiness_summaries"][0]
    ...
    assert branch == summary_payload
    assert branch["summary_state"] == "candidate_only"
    assert branch["required_control_count"] == 6
    assert branch["satisfied_control_count"] == 1
    assert branch["missing_control_count"] == 5
    assert branch["diagnostic_limitations"]
    assert "source_readiness" not in branch
    assert "research_data_source_readiness" not in payload
    assert "\n" not in output
    assert '": ' not in output


def test_readiness_summary_cli_outputs_are_byte_deterministic(capsys) -> None:
    first_text = _run_preview_cli((_COMMAND, _SUMMARY_FLAG), capsys)
    second_text = _run_preview_cli((_COMMAND, _SUMMARY_FLAG), capsys)
    first_json = _run_preview_cli(
        (_COMMAND, _SUMMARY_FLAG, "--format", "json"),
        capsys,
    )
    second_json = _run_preview_cli(
        (_COMMAND, "--format", "json", _SUMMARY_FLAG),
        capsys,
    )

    assert first_text == second_text == _expected_summary_export().rendered_text
...
def test_all_flags_keep_readiness_summary_after_existing_diagnostic_branch(
    capsys,
) -> None:
    expected = _expected_summary_export(
    ...
    assert lines.index("Research Data Source Readiness Summary Diagnostics") < (
        lines.index("Limitations")
    )
...
def test_readiness_summary_branch_adds_no_runtime_trading_or_vendor_fields(
    capsys,
) -> None:
    payload = json.loads(
        _run_preview_cli(
    ...
    summary_payloads = payload["research_data_source_readiness_summaries"]
    field_names = _serialized_keys(summary_payloads)
