...
def test_valid_local_csv_and_valid_manifest_writes_canonical_and_refreshes(
    ...
    assert payload["provenance_validation"]["expected_input_sha256"] == (
        payload["data_validation"]["input_sha256"]
    )
    assert payload["provenance_validation"]["input_sha256_matches_manifest"] is True
...
def test_manifest_path_mismatch_blocks(tmp_path) -> None:  # noqa: ANN001
    ...
    manifest = _write_manifest(
        ...
        updates={"expected_input_sha256": _sha256_file(csv_path)},
    )
...
def test_missing_expected_input_sha256_blocks_without_canonical_output(
    tmp_path,
) -> None:  # noqa: ANN001
    csv_path = _write_csv(
        tmp_path / "operator_evidence" / "spy_daily_bars.csv",
    ...
    assert "missing_provenance_field:expected_input_sha256" in payload["blockers"]
    assert payload["provenance_validation"]["valid"] is False
    assert payload["provenance_validation"]["input_sha256_matches_manifest"] is False
    assert payload["canonical_csv_written"] is False
    assert not canonical_output.exists()
...
def test_invalid_expected_input_sha256_blocks(
    tmp_path,
    ...
    manifest = _write_manifest(
        tmp_path / "manifest.json",
        csv_path,
        updates={"expected_input_sha256": expected_input_sha256},
    )
...
def test_expected_input_sha256_mismatch_blocks(tmp_path) -> None:  # noqa: ANN001
    csv_path = _write_csv(
        tmp_path / "operator_evidence" / "spy_daily_bars.csv",
        150 * ("100",) + 50 * ("200",) + ("220",),
    )
    manifest = _write_manifest(
        tmp_path / "manifest.json",
        csv_path,
        updates={"expected_input_sha256": "0" * 64},
    )
    ...
    assert payload["provenance_validation"]["expected_input_sha256"] == "0" * 64
    assert payload["provenance_validation"]["input_sha256_matches_manifest"] is False
    assert payload["data_validation"]["input_sha256"] != "0" * 64
    assert payload["canonical_csv_written"] is False
...
@pytest.mark.parametrize(
    ("field", "value"),
    (
        ("source_type", "codex_generated"),
        ("source_type", "synthetic_sample"),
        ("acquisition_method", "auto generated"),
        ("source_description", "generated by codex"),
        ("notes", "model generated sample"),
    ),
)
def test_compound_or_phrase_forbidden_provenance_terms_block(
    tmp_path,
    field: str,
    value: str,
) -> None:  # noqa: ANN001
...
def _write_manifest(
    ...
    *,
    ...
    record: dict[str, object] = {
        ...
        "expected_input_sha256": (
            _sha256_file(input_csv) if input_csv.is_file() else "0" * 64
        ),
        "notes": "unit manifest; not operator evidence",
    }
    if updates:
