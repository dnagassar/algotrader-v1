...
SCRIPT = PROJECT_ROOT / "scripts" / "run_paper_autopilot_operator.ps1"


def test_run_paper_autopilot_operator_script_contract() -> None:
...
def test_run_paper_autopilot_operator_script_invokes_cli_with_paths(
    tmp_path: Path,
) -> None:
    output_root = tmp_path / "paper autopilot latest"
    history_root = tmp_path / "paper autopilot history"
...
def _fake_python_env(tmp_path: Path, capture_path: Path) -> dict[str, str]:
    ...
    fake_python.write_text(
        ...
        "echo {\"classification\":\"healthy_hold_noop\"}\r\n"
        "exit /B 0\r\n",
        encoding="utf-8",
        newline="",
    )
...
def _powershell() -> str:
    ...
    if powershell is None:
        pytest.skip("PowerShell is required to verify run_paper_autopilot_operator.ps1")
    return powershell
