...
def test_report_consumes_phase_78_fixture_and_summarizes_shape_metadata() -> None:
    ...
    assert_non_claims_include_not_evidence_approval(
        report["methodology_non_claims"],
        context="report methodology_non_claims",
    )
...
def test_report_output_is_deterministic_json_primitive_and_byte_stable() -> None:
    ...
    assert_json_payload_uses_only_primitives(
        first,
        context="planning replay report payload",
    )
...
def test_report_keeps_planning_approval_states_candidate_blocked_or_deferred() -> None:
    ...
    planning_states = tuple(report["planning_approval_states"])

    assert_planning_states_are_non_approved(
        planning_states,
        context="report planning_approval_states",
    )


@pytest.mark.parametrize("approval_state", REJECTED_PLANNING_STATE_EXAMPLES)
def test_report_rejects_planning_approval_states_outside_the_allowed_set(
    approval_state: str,
) -> None:
...
def test_report_preserves_linked_scope_ids_and_evidence_refs_as_metadata_only() -> None:
    ...
    scrubbed_json = scrub_negative_assertions(compact_json, report)
...
def test_report_false_interpretation_guardrails_are_explicitly_negative() -> None:
    ...
    assert_required_non_claims_present(
        non_claims,
        _REQUIRED_NON_CLAIMS,
        context="planning replay report non_claims",
    )
    for phrase in (
        "not a strategy validation artifact",
        "not a validated research artifact",
    ...
    assert_no_forbidden_terms(
        scrubbed_json,
        _FORBIDDEN_INTERPRETATION_TERMS,
        context="planning replay report scrubbed JSON",
    )


def test_report_does_not_add_replay_metrics_or_trading_behavior_fields() -> None:
    report = build_synthetic_broad_etf_planning_replay_report()
    keys = all_serialized_keys(report)
    compact_json = json.dumps(report, separators=(",", ":")).lower()
    scrubbed_json = scrub_negative_assertions(compact_json, report)
...
def test_report_introduces_no_real_data_tickers_vendors_paths_or_credentials() -> None:
    ...
    scrubbed_json = scrub_negative_assertions(lowered_json, report)
    ...
    assert_no_real_etf_tickers(compact_json, context="planning replay report JSON")
    assert_no_real_vendor_or_source_identifiers(
        compact_json,
        context="planning replay report JSON",
    )
