...
def build_parser() -> argparse.ArgumentParser:
    ...
    etf_sma_daily_artifact_manifest_health_parser = subparsers.add_parser(
        "etf-sma-daily-artifact-manifest-health",
        help="Validate manifest health for daily cycle artifacts (M447-M453) offline.",
    )
    etf_sma_daily_artifact_manifest_health_parser.add_argument(
        "--m447-jsonl",
        default="runs/paper_lab/m447_offline_daily_cycle_m446_rerun_manifest.jsonl",
        help="Path to the M447 manifest JSONL.",
    )
    ...
    etf_sma_daily_artifact_manifest_health_parser.add_argument(
        ...
        help="Path to write the M454 daily manifest health JSONL.",
    )
    etf_sma_daily_artifact_manifest_health_parser.add_argument(
        "--format",
        choices=_PREVIEW_FORMATS,
    ...
    etf_sma_daily_paper_lab_parser.add_argument(
        ...
        help=(
            "Broker-state lane mode. alpaca_paper_read_only consumes only an "
            "already-local read-only snapshot artifact and performs no broker read."
        ),
    )
    etf_sma_daily_paper_lab_parser.add_argument(
        "--broker-snapshot-log",
    ...
    paper_autopilot_operator_parser = subparsers.add_parser(
        "paper-autopilot-operator",
        help="Run paper-autopilot loop, history, and operator status classification.",
    )
    ...
    paper_autopilot_operator_parser.add_argument("--as-of-date", default=None)
    paper_autopilot_operator_parser.add_argument("--run-date", default=None)
    paper_autopilot_operator_parser.add_argument("--symbol", default="SPY")
    paper_autopilot_operator_parser.add_argument(
        "--sma-fast-window",
        type=int,
        default=50,
    ...
    etf_sma_daily_soak_brief_parser = subparsers.add_parser(
        "etf-sma-daily-soak-brief",
        help="Build a deterministic offline operator brief command that consumes daily soak outputs and daily bundle artifacts.",
    )
    etf_sma_daily_soak_brief_parser.add_argument(
        "--soak-rollup-jsonl",
        required=True,
        help="Path to the soak rollup JSONL file.",
    )
    etf_sma_daily_soak_brief_parser.add_argument(
        "--daily-root",
        required=True,
        help="Path to the daily root directory.",
    )
    etf_sma_daily_soak_brief_parser.add_argument(
        "--output-jsonl",
        default="runs/daily_soak/soak_operator_brief.jsonl",
        help="Path to write the brief JSONL rollup record.",
    )
    etf_sma_daily_soak_brief_parser.add_argument(
        "--output-text",
        default="runs/daily_soak/soak_operator_brief.txt",
        help="Path to write the brief text status report.",
    )
    etf_sma_daily_soak_brief_parser.add_argument(
        "--baseline-rollup-jsonl",
        default=None,
        help="Optional path to a baseline soak rollup JSONL for regression comparison.",
    )
    etf_sma_daily_soak_brief_parser.add_argument(
        "--format",
        choices=_PREVIEW_FORMATS,
    ...
    etf_sma_daily_soak_closeout_catalog_parser = subparsers.add_parser(
        "etf-sma-daily-soak-closeout-catalog",
        help="Create a deterministic offline catalog/index of closeout bundles (V3Q).",
    )
    etf_sma_daily_soak_closeout_catalog_parser.add_argument(
        "--bundle-root",
        action="append",
        default=[],
        help="One or more daily closeout bundle roots to catalog.",
    )
    etf_sma_daily_soak_closeout_catalog_parser.add_argument(
        "--output-jsonl",
        default="runs/daily_soak/closeout_catalog.jsonl",
        help="Path to write the catalog JSONL records.",
    )
    etf_sma_daily_soak_closeout_catalog_parser.add_argument(
        "--output-text",
        default="runs/daily_soak/closeout_catalog.md",
        help="Path to write the catalog markdown summary.",
    )
    etf_sma_daily_soak_closeout_catalog_parser.add_argument(
        "--format",
        choices=_PREVIEW_FORMATS,
    ...
    # V5.33 subcommands
    crypto_verify_parser = subparsers.add_parser(
        "crypto-readiness-verify",
        help="Run offline verification of mock fixture/replay data.",
    )
    crypto_verify_parser.add_argument("--output-root", default="runs/crypto_supervised_readiness_trial/latest")
    crypto_verify_parser.add_argument("--cycle-count", type=int, default=24)

    crypto_preflight_parser = subparsers.add_parser(
        "crypto-readiness-preflight",
        help="Run preflight check asserting profile and credentials are NOT present in offline mode.",
    )

    crypto_observe_parser = subparsers.add_parser(
    ...
    crypto_consume_parser = subparsers.add_parser(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if command == "etf-sma-daily-soak-closeout-catalog":
        return _run_etf_sma_daily_soak_closeout_catalog(args)
    ...
    if command == "crypto-readiness-verify":
        return _run_crypto_readiness_verify(args)
    if command == "crypto-readiness-preflight":
        return _run_crypto_readiness_preflight(args)
    if command == "crypto-paper-broker-observation":
...
def _run_etf_sma_daily_artifact_manifest_health(args: argparse.Namespace) -> int:
    from .errors import ValidationError
    from .execution.etf_sma_daily_artifact_manifest_health import (
    ...
    try:
        payload = run_etf_sma_daily_artifact_manifest_health(
            EtfSmaDailyArtifactManifestHealthConfig(
                m447_jsonl=args.m447_jsonl,
                m448_jsonl=args.m448_jsonl,
                m449_jsonl=args.m449_jsonl,
...
def _run_paper_autopilot_operator(args: argparse.Namespace) -> int:
    import json
    import sys
    from .errors import ValidationError
    from .execution.paper_autopilot_operator import (
    ...
    try:
        ...
        if args.output_format == "json":
            ...
            print(
                render_paper_autopilot_operator_summary(result["operator_summary"]),
                end="",
            )
...
def _run_etf_sma_daily_soak_brief(args: argparse.Namespace) -> int:
    import json
    import sys
    from .errors import ValidationError
    from .execution.etf_sma_daily_soak_brief import (
        EtfSmaDailySoakBriefConfig,
        run_etf_sma_daily_soak_brief,
    )

    try:
        payload = run_etf_sma_daily_soak_brief(
            EtfSmaDailySoakBriefConfig(
                soak_rollup_jsonl=args.soak_rollup_jsonl,
                daily_root=args.daily_root,
                output_jsonl=args.output_jsonl,
                output_text=args.output_text,
                baseline_rollup_jsonl=args.baseline_rollup_jsonl,
...
def _run_etf_sma_daily_soak_closeout_catalog(args: argparse.Namespace) -> int:
    import json
    import sys
    from .errors import ValidationError
    from .execution.etf_sma_daily_soak_closeout_catalog import (
        DailyLabCloseoutCatalogConfig,
        run_daily_lab_closeout_catalog,
    )

    bundle_roots = args.bundle_root if args.bundle_root else ["runs/daily_soak"]

    try:
        records = run_daily_lab_closeout_catalog(
            DailyLabCloseoutCatalogConfig(
...
def _run_crypto_readiness_verify(args: argparse.Namespace) -> int:
    from .execution.crypto_supervised_readiness_trial import run_crypto_supervised_readiness_trial
    packet = run_crypto_supervised_readiness_trial(
        output_root=args.output_root,
        cycle_count=args.cycle_count,
        write_artifacts=True,
    )
    print(f"crypto_verify_classification={packet['trial_classification']}")
    return 0 if packet["trial_classification"] == "accepted" else 2


def _run_crypto_readiness_preflight(args: argparse.Namespace) -> int:
    import os
    ...
    if profile == "paper" or credentials_present:
        print("crypto_preflight_status=blocked_unsafe_offline_environment")
        return 2
    print("crypto_preflight_status=preflight_success")
    return 0
...
def _run_crypto_readiness_consume(args: argparse.Namespace) -> int:
    from .execution.crypto_supervised_readiness_trial import run_crypto_supervised_readiness_trial
    packet = run_crypto_supervised_readiness_trial(
        output_root=args.output_root,
        write_artifacts=True,
