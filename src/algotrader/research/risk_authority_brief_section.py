"""Metadata-only advisory risk authority brief section contract."""
...
_SECTION_TYPE = "risk_authority_brief_section"
...
class RiskAuthorityBriefSection:
    """Primitive advisory metadata grouping risk authority brief items."""
...
def build_risk_authority_brief_section(
    items: Iterable[RiskAuthorityBriefItem],
) -> RiskAuthorityBriefSection:
    """Build a deterministic advisory-only risk authority section."""

    checked_items = _items_tuple(items)
    return RiskAuthorityBriefSection(
...
def _title(items: tuple[RiskAuthorityBriefItem, ...]) -> str:
    if len(items) == 1:
        return f"Advisory risk metadata: {items[0].authority_state}"

    return f"Advisory risk metadata: {len(items)} items"


def _summary(items: tuple[RiskAuthorityBriefItem, ...]) -> str:
    strategy_ids = _unique_string_values(
        strategy_id
        for item in items
        for strategy_id in item.related_strategy_ids
    )
    states = _unique_string_values(item.authority_state for item in items)
    limitations = _combined_item_values(items, "limitations")
    non_claims = _combined_item_values(items, "non_claims")
    return (
        f"Advisory section contains {len(items)} candidate risk metadata item(s) "
        f"across {len(strategy_ids)} related strategy id(s), state(s): "
...
def _items_tuple(
    values: Iterable[RiskAuthorityBriefItem],
) -> tuple[RiskAuthorityBriefItem, ...]:
    try:
        items = tuple(values)
    except TypeError as exc:
        raise ValidationError(
            "items must be an iterable of RiskAuthorityBriefItem."
        ) from exc
    ...
    for index, item in enumerate(items):
        if type(item) is not RiskAuthorityBriefItem:
            raise ValidationError(
                f"items[{index}] must be a RiskAuthorityBriefItem."
            )
...
def _validate_fixed_metadata(
    ...
    if section_type != _SECTION_TYPE:
        raise ValidationError(
            "section_type must be exactly risk_authority_brief_section."
        )
    if status != _STATUS:
