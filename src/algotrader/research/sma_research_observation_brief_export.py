"""Deterministic in-memory export for SMA research observation briefs."""
...
class SmaResearchObservationBriefExport:
    """Primitive payload, compact JSON, and rendered text for an SMA brief."""
...
def export_sma_research_observation_brief(
    brief: SmaResearchObservationBrief,
) -> SmaResearchObservationBriefExport:
    """Return deterministic in-memory export views for an existing SMA brief."""
    ...
    return SmaResearchObservationBriefExport(
        payload=payload,
        json_text=json.dumps(payload, sort_keys=True, separators=(",", ":")),
        rendered_text=render_sma_research_observation_brief_text(brief),
    )
