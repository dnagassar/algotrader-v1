"""Frozen V5.86 Clare risk-parity trend-proxy tournament."""
...
__all__ = [
    "build_clare_risk_parity_trend_preregistration",
    "run_clare_risk_parity_trend_tournament",
]

PROTOCOL_ID = "v5_86_clare_risk_parity_trend_v1"
CANDIDATE_ID = "clare_five_asset_inverse_vol_10m_trend_proxy"
ABLATION_ID = "inverse_volatility_no_trend"
STATIC_ID = "static_equal_risky_monthly"
BIL_ID = "bil_buy_and_hold"
SPY_ID = "spy_buy_and_hold"
PARENT_ID = "spy_ief_60_40_monthly"
RISK_SYMBOLS = ("URTH", "VWO", "BND", "DBC", "VNQ")
ALL_SYMBOLS = (*RISK_SYMBOLS, "BIL", "SPY", "IEF")
_PROTOCOL = Path("docs/design/v5_86_clare_risk_parity_trend_preregistration.md")
_RECEIPT = Path("docs/design/v5_86_clare_risk_parity_trend_data_receipt.md")
_DATA = Path("runs/v5_86_clare_risk_parity_trend/canonical_data.csv")
_DATA_MANIFEST = Path("runs/v5_86_clare_risk_parity_trend/canonical_data_manifest.json")
_OUTPUT = Path("runs/v5_86_clare_risk_parity_trend/evaluation")
_ENGINE = Path("src/algotrader/research/clare_risk_parity_trend_tournament.py")
_PROTOCOL_HASH = "b457cd6704f61f5332edce25ba357303e55245dd878de03c0420b80aedcfacd4"
_RECEIPT_HASH = "c295d2a4160be3cabcd42e1cedc3096dd83033182904ec9c83f148073eb809e4"
_DATA_HASH = "72dbc83f4523f9e003a0fce87add0f6c341edaec3efdeafce90e9a17d6d133a2"
_MANIFEST_HASH = "303f5827113a304b5262d7920de25c3d80482d03fbba8f2e4e329e1ba43c78cd"
_START = date(2012, 1, 12)
_END = date(2026, 7, 31)
_OOS_START = date(2016, 4, 1)
_WINDOWS = (
    ("oos", _OOS_START, _END),
    ("oos_fold_1", date(2016, 4, 1), date(2019, 9, 30)),
    ("oos_fold_2", date(2019, 10, 1), date(2023, 3, 31)),
    ("oos_fold_3", date(2023, 4, 3), _END),
)
_SESSION_COUNTS = {
    "all": 3658,
    "oos": 2598,
...
def build_clare_risk_parity_trend_preregistration() -> dict[str, object]:
    _validate_tracked_inputs()
    return {
        "record_type": "clare_risk_parity_trend_preregistration",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "candidate_ids": [CANDIDATE_ID],
        "control_ids": [ABLATION_ID, STATIC_ID, BIL_ID],
        "baseline_ids": [SPY_ID, PARENT_ID],
        "symbols": list(ALL_SYMBOLS),
        "risk_symbols": list(RISK_SYMBOLS),
        "oos_start": _OOS_START.isoformat(),
        "oos_end": _END.isoformat(),
        "volatility_lookback_monthly_returns": 12,
        "trend_lookback_month_end_levels": 10,
        "volatility_estimator": "sample_standard_deviation_n_minus_1",
        "trend_rule": "strictly_above_arithmetic_mean",
        "downtrend_allocation": "transfer_full_prefilter_weight_to_BIL",
...
def run_clare_risk_parity_trend_tournament(output_root: Path | str = _OUTPUT) -> dict[str, object]:
    preregistration = build_clare_risk_parity_trend_preregistration()
...
def _build_result(
    ...
    *,
    ...
    return {
        "record_type": "clare_risk_parity_trend_tournament_result",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
...
def _build_actions(data: _Data) -> dict[str, dict[date, dict[str, float]]]:
    ...
    for month_position, end_index in enumerate(month_ends):
        if month_position < 12 or end_index + 1 >= len(data.dates):
            continue
        ...
        for symbol in RISK_SYMBOLS:
            monthly_returns = [
                data.prices[symbol][month_ends[position]]
                / data.prices[symbol][month_ends[position - 1]]
                - 1.0
                for position in range(month_position - 11, month_position + 1)
            ]
            volatility = stdev(monthly_returns)
            if not math.isfinite(volatility) or volatility <= 0.0:
                raise ValidationError(f"{symbol} monthly volatility is invalid.")
            volatilities[symbol] = volatility

        inverse_total = sum(1.0 / value for value in volatilities.values())
        prefilter = {
            symbol: (1.0 / volatilities[symbol]) / inverse_total
            for symbol in RISK_SYMBOLS
        }
        ...
        for symbol in RISK_SYMBOLS:
            ablation[symbol] = prefilter[symbol]
            recent_month_ends = month_ends[month_position - 9 : month_position + 1]
            moving_average = mean(
                data.prices[symbol][index] for index in recent_month_ends
            )
            if data.prices[symbol][end_index] > moving_average:
                candidate[symbol] = prefilter[symbol]
        candidate["BIL"] = 1.0 - sum(
            candidate[symbol] for symbol in RISK_SYMBOLS
        )
    ...
    result[BIL_ID][_OOS_START] = {**_empty_target(), "BIL": 1.0}
...
def _evaluate(
    ...
    comparator_ids = (ABLATION_ID, STATIC_ID, BIL_ID, SPY_ID, PARENT_ID)
    comparators = {
        strategy_id: {
    ...
    candidate = {
        ...
        "target_contract": {
            ...
            "max_risky_target_weight": _number(
                max(
                    target[symbol]
            ...
            "divergent_decisions_from_inverse_volatility": _divergence(
    ...
    integrity = {
        ...
        "twelve_month_sample_volatility_verified": True,
        "ten_month_strict_trend_filter_verified": True,
        "downtrend_weight_transfers_to_bil_without_renormalization": True,
...
def _decisions(
    ...
    *,
    ...
    common = {
        ...
        "all_risky_sleeves_held": all(
            symbol in oos["symbols_held"] for symbol in RISK_SYMBOLS
        ),
        "at_least_three_risky_sleeves_contribute_positively": sum(
            contributions[symbol] > 0.0 for symbol in RISK_SYMBOLS
        )
        >= 3,
        "max_risky_target_at_most_0_60": _float(
            item["target_contract"]["max_risky_target_weight"]
        )
        <= 0.60,
    ...
    ablation_gate = {
        "sharpe_exceeds_inverse_volatility_by_0_05": _required_float(
            ablation["oos"]["sharpe_ratio_delta"]
        )
        >= 0.05,
        "drawdown_improves_by_10pct_relative": _float(
            ablation["oos"]["max_drawdown_improvement"]
        )
        >= 0.10 * ablation_drawdown,
        ...
        "at_least_12_divergent_monthly_decisions": (
            item["target_contract"][
                "divergent_decisions_from_inverse_volatility"
            ]
            >= 12
        ),
    }
...
def _artifact_manifest(result: Mapping[str, object]) -> dict[str, object]:
    return {
        "record_type": "clare_risk_parity_trend_artifact_manifest",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "inputs": {
            "protocol_sha256": _PROTOCOL_HASH,
...
def _summary(result: Mapping[str, object]) -> str:
    ...
    lines = [
        "# V5.86 Clare risk-parity trend tournament result",
        "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_clare_risk_parity_trend_tournament(args.output_root)
    except (OSError, ValidationError, ValueError) as exc:
        print(f"clare_risk_parity_trend_tournament_status=blocked:{exc}")
        return 2
    print("clare_risk_parity_trend_tournament_status=completed")
