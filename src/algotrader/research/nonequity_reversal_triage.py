"""Frozen V5.93 non-equity short-term reversal triage.

One exactly specified short-term reversal rule is applied identically to
eighteen non-equity markets this repository had never acquired. The rule is the
mechanical opposite of V5.91's trend rule: it holds a market precisely when that
market has recently fallen.

Commodities, currencies, and credit carry no sustained bull market over this
window, so a failure here cannot be explained by a rising benchmark — the
explanation available for both prior equity triages.
"""
...
__all__ = [
    "build_reversal_triage_preregistration",
    "run_nonequity_reversal_triage",
]

PROTOCOL_ID = "v5_93_nonequity_reversal_triage_v1"
RULE_ID = "short_term_reversal_21_session_negative_return_monthly"
...
_RECEIPT = Path(
    "docs/design/v5_93_nonequity_reversal_triage_data_receipt.md"
)
_ROOT = Path("runs/v5_93_nonequity_reversal_triage")
_DATA = _ROOT / "canonical_data.csv"
_DATA_MANIFEST = _ROOT / "canonical_data_manifest.json"
_OUTPUT = _ROOT / "evaluation"
_ENGINE = Path("src/algotrader/research/nonequity_reversal_triage.py")
_PROTOCOL_HASH = "0cc32975be515949fe64f053cd63e5e1917eb1cd0429fdb6dd31a2e02694cd62"
_RECEIPT_HASH = "e61592bab330600db2620c26ac8381ee6756bedc8426934a53886ef2df77078a"
_DATA_HASH = "953580d614bb3908620786e3fb9e8ee29dcdfab23dcf4cb19c2edaa0f99e1c06"
_MANIFEST_HASH = "b94eac83a9378f211836d8ea1749bd8d6beeddc6ff7bfd4cb1245f5b73298291"

_LOOKBACK = 21
_WARM_UP = 22
_END = date(2026, 7, 31)
_SESSION_COUNT = 4403
_SCORED_SESSION_COUNT = 4381
_DECISIONS_PER_MARKET = 208
...
def build_reversal_triage_preregistration() -> dict[str, object]:
    _validate_tracked_inputs()
    return {
        "record_type": "reversal_triage_preregistration",
        "schema_version": 1,
        ...
        "signal": "trailing_21_session_simple_return_strictly_below_zero",
...
def run_nonequity_reversal_triage(
    output_root: Path | str = _OUTPUT,
) -> dict[str, object]:
    preregistration = build_reversal_triage_preregistration()
...
def _evaluate_market(
    ...
    held_months = sum(1 for value in actions.values() if value > 0.0)
    return (
        {
            "market": market,
            ...
            "months_held": held_months,
            "months_in_cash": len(actions) - held_months,
            "held_month_fraction": _number(held_months / len(actions)),
        },
        excess,
    )
...
def _build_actions(
    ...
    """Hold the market only after a negative trailing 21-session return."""
    ...
    for index in range(len(dates) - 1):
        ...
        if index < _WARM_UP - 1:
            continue
        prior = closes[index - _LOOKBACK]
        if not math.isfinite(prior) or prior <= 0.0:
            raise ValidationError("reversal lookback close is invalid.")
        trailing = closes[index] / prior - 1.0
        if not math.isfinite(trailing):
            raise ValidationError("trailing return is nonfinite.")
        actions[dates[index + 1]] = 1.0 if trailing < 0.0 else 0.0
    return actions
...
def _build_result(
    ...
    return {
        "record_type": "nonequity_reversal_triage_result",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
...
def _artifact_manifest(result: Mapping[str, object]) -> dict[str, object]:
    return {
        "record_type": "reversal_triage_artifact_manifest",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "inputs": {
            "protocol_sha256": _PROTOCOL_HASH,
...
def _summary(result: Mapping[str, object]) -> str:
    ...
    lines = [
        "# V5.93 non-equity short-term reversal triage result",
        "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_nonequity_reversal_triage(args.output_root)
    except (OSError, ValidationError, ValueError) as exc:
        print(f"reversal_triage_status=blocked:{exc}")
        return 2
    cross = result["cross_section"]
    print("reversal_triage_status=completed")
