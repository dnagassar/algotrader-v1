"""Metadata-only advisory research queue brief contract."""
...
_BRIEF_TYPE = "research_queue_brief"
...
class ResearchQueueBrief:
    """Primitive advisory metadata grouping research queue brief sections."""
    ...
    title: str
    summary: str
    sections: tuple[ResearchQueueBriefSection, ...]
...
def build_research_queue_brief(
    sections: Iterable[ResearchQueueBriefSection],
) -> ResearchQueueBrief:
    """Build a deterministic advisory-only research queue brief."""

    checked_sections = _sections_tuple(sections)
    return ResearchQueueBrief(
...
def _title(sections: tuple[ResearchQueueBriefSection, ...]) -> str:
    if len(sections) == 1:
        return "Research queue brief: 1 section"

    return f"Research queue brief: {len(sections)} sections"


def _summary(sections: tuple[ResearchQueueBriefSection, ...]) -> str:
    ...
    return (
        f"Research queue brief contains {len(sections)} candidate metadata "
...
def _sections_tuple(
    values: Iterable[ResearchQueueBriefSection],
) -> tuple[ResearchQueueBriefSection, ...]:
    try:
        sections = tuple(values)
    except TypeError as exc:
        raise ValidationError(
            "sections must be an iterable of ResearchQueueBriefSection."
        ) from exc
    ...
    for index, section in enumerate(sections):
        if type(section) is not ResearchQueueBriefSection:
            raise ValidationError(
                f"sections[{index}] must be a ResearchQueueBriefSection."
            )
...
def _validate_fixed_metadata(
    ...
    if brief_type != _BRIEF_TYPE:
        raise ValidationError("brief_type must be exactly research_queue_brief.")
    if status != _STATUS:
