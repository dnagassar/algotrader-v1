"""Metadata-only advisory container for SMA research observation sections."""
...
_BRIEF_TYPE = "sma_research_observation_brief"
...
class SmaResearchObservationBrief:
    """Primitive advisory metadata grouping SMA observation brief sections."""
    ...
    title: str
    summary: str
    sections: tuple[SmaResearchObservationBriefSection, ...]
...
def build_sma_research_observation_brief(
    brief_id: str,
    title: str,
    summary: str,
    sections: Iterable[SmaResearchObservationBriefSection],
) -> SmaResearchObservationBrief:
    """Build a deterministic advisory-only SMA observation brief."""
...
def _sections_tuple(
    values: Iterable[SmaResearchObservationBriefSection],
) -> tuple[SmaResearchObservationBriefSection, ...]:
    try:
        sections = tuple(values)
    except TypeError as exc:
        raise ValidationError(
            "sections must be an iterable of SmaResearchObservationBriefSection."
        ) from exc
    ...
    for index, section in enumerate(sections):
        if type(section) is not SmaResearchObservationBriefSection:
            raise ValidationError(
                f"sections[{index}] must be a SmaResearchObservationBriefSection."
            )
...
def _validate_fixed_metadata(
    ...
    if brief_type != _BRIEF_TYPE:
        raise ValidationError(
            "brief_type must be exactly sma_research_observation_brief."
        )
    if status != _STATUS:
