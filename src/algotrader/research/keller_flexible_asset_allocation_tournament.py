"""Frozen V5.87 Keller flexible asset allocation-proxy tournament."""
...
__all__ = [
    "build_keller_flexible_asset_allocation_preregistration",
    "run_keller_flexible_asset_allocation_tournament",
]

PROTOCOL_ID = "v5_87_keller_faa_v3"
CANDIDATE_ID = "keller_faa_ravc_4m_top3_proxy"
ABLATION_ID = "relative_absolute_momentum_top3_4m"
STATIC_ID = "static_equal_seven_monthly"
SHY_ID = "shy_buy_and_hold"
SPY_ID = "spy_buy_and_hold"
PARENT_ID = "spy_ief_60_40_monthly"
CANDIDATE_SYMBOLS = ("VTI", "VEA", "VWO", "SHY", "BND", "GSG", "VNQ")
RISK_SYMBOLS = ("VTI", "VEA", "VWO", "BND", "GSG", "VNQ")
ALL_SYMBOLS = (*CANDIDATE_SYMBOLS, "SPY", "IEF")
_PROTOCOL = Path(
    "docs/design/v5_87_keller_flexible_asset_allocation_preregistration.md"
)
_RECEIPT = Path(
    "docs/design/v5_87_keller_flexible_asset_allocation_data_receipt.md"
)
_DATA = Path("runs/v5_87_keller_flexible_asset_allocation/canonical_data.csv")
_DATA_MANIFEST = Path(
    "runs/v5_87_keller_flexible_asset_allocation/canonical_data_manifest.json"
)
_OUTPUT = Path("runs/v5_87_keller_flexible_asset_allocation/evaluation")
_ENGINE = Path(
    "src/algotrader/research/keller_flexible_asset_allocation_tournament.py"
)
_PROTOCOL_HASH = "c69692a82d58376be64a05118e73af77604e6ed6495987ed549e0b57ace2aff7"
_RECEIPT_HASH = "c338615d6079557b3a5d98dd0414cef7a15f03e06812841af5ddadb63f30fa60"
_DATA_HASH = "5094981d3c24aa6d018123b6aad20ce9e70583ed09ff6df23778c64ec65c2502"
_MANIFEST_HASH = "e911031d7ab3f9bc9669643ed2ee19de369daa2c1a15492391a04e9231becd08"
_START = date(2007, 7, 26)
_END = date(2026, 7, 31)
_OOS_START = date(2013, 2, 1)
_WINDOWS = (
    ("oos", _OOS_START, _END),
    ("oos_fold_1", date(2013, 2, 1), date(2017, 6, 30)),
...
def build_keller_flexible_asset_allocation_preregistration() -> dict[str, object]:
    _validate_tracked_inputs()
    return {
        "record_type": "keller_flexible_asset_allocation_preregistration",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "candidate_ids": [CANDIDATE_ID],
        "control_ids": [ABLATION_ID, STATIC_ID, SHY_ID],
        "baseline_ids": [SPY_ID, PARENT_ID],
        "symbols": list(ALL_SYMBOLS),
        "candidate_symbols": list(CANDIDATE_SYMBOLS),
        "non_cash_symbols": list(RISK_SYMBOLS),
        "oos_start": _OOS_START.isoformat(),
        "oos_end": _END.isoformat(),
        "lookback_calendar_months": 4,
        "daily_volatility_estimator": "sample_standard_deviation_n_minus_1",
        "correlation_estimator": "sample_pearson_daily_returns",
        "score_weights": {
        ...
        "selected_weight": "0.333333333333",
        "factor_tie_rule": "average_ordinal_rank",
        "candidate_score_tie_rule": "published_etf_order",
        "ablation_return_tie_rule": "published_etf_order",
        "negative_return_substitute": "SHY",
...
def run_keller_flexible_asset_allocation_tournament(output_root: Path | str = _OUTPUT) -> dict[str, object]:
    preregistration = build_keller_flexible_asset_allocation_preregistration()
...
def _build_result(
    ...
    *,
    ...
    decision = _decisions(evaluation, replay_equal=replay_equal)
    winner = CANDIDATE_ID if decision["all_gates_passed"] else None
    return {
        "record_type": "keller_flexible_asset_allocation_tournament_result",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        ...
        "tournament_decision": {
            ...
            "passing_candidate_count": 1 if winner else 0,
...
def _load_data() -> _Data:
    ...
    if (
        ...
        or frozen_pins.get("protocol_sha256") != _PROTOCOL_HASH
    ):
...
def _build_actions(data: _Data) -> dict[str, dict[date, dict[str, float]]]:
    ...
    for month_position, end_index in enumerate(month_ends):
        if month_position < 4 or end_index + 1 >= len(data.dates):
            continue
        action_date = data.dates[end_index + 1]
        if not (_OOS_START <= action_date <= _END):
            continue

        start_index = month_ends[month_position - 4]
        momentum = {
            symbol: (
        ...
        volatilities = {
            symbol: stdev(daily_returns[symbol])
            for symbol in CANDIDATE_SYMBOLS
        }
        if any(
            ...
            raise ValidationError("four-month daily volatility is invalid.")
        average_correlations = {
            symbol: mean(
                _pearson(daily_returns[symbol], daily_returns[other])
                for other in CANDIDATE_SYMBOLS
                if other != symbol
            )
            for symbol in CANDIDATE_SYMBOLS
        }

        return_ranks = _average_ranks(momentum, higher_is_better=True)
        volatility_ranks = _average_ranks(
            volatilities, higher_is_better=False
        )
        correlation_ranks = _average_ranks(
            average_correlations, higher_is_better=False
        )
        scores = {
            symbol: (
                return_ranks[symbol]
        ...
        selected = sorted(
            CANDIDATE_SYMBOLS,
            key=lambda symbol: (scores[symbol], published_order[symbol]),
        )[:3]
        return_selected = sorted(
            CANDIDATE_SYMBOLS,
            key=lambda symbol: (
                -momentum[symbol],
                published_order[symbol],
            ),
        )[:3]

        candidate = _empty_target()
        ablation = _empty_target()
        for symbol in selected:
            destination = "SHY" if momentum[symbol] < 0.0 else symbol
            candidate[destination] += 1.0 / 3.0
        for symbol in return_selected:
            destination = "SHY" if momentum[symbol] < 0.0 else symbol
            ablation[destination] += 1.0 / 3.0

        static = _empty_target()
        for symbol in CANDIDATE_SYMBOLS:
            static[symbol] = 1.0 / len(CANDIDATE_SYMBOLS)
...
def _validate_action_contract(
    ...
    monthly_ids = (CANDIDATE_ID, ABLATION_ID, STATIC_ID, PARENT_ID)
    for strategy_id in monthly_ids:
        if tuple(sorted(actions[strategy_id])) != expected_dates:
            raise ValidationError(
                f"{strategy_id} monthly action dates do not match the contract."
            )
        for target in actions[strategy_id].values():
            _validate_weights(target)
    if tuple(sorted(actions[SHY_ID])) != (_OOS_START,):
        raise ValidationError("SHY buy-and-hold action date mismatch.")
    if tuple(sorted(actions[SPY_ID])) != (_OOS_START,):
        raise ValidationError("SPY buy-and-hold action date mismatch.")
    for strategy_id in (SHY_ID, SPY_ID):
        _validate_weights(actions[strategy_id][_OOS_START])
    for target in actions[CANDIDATE_ID].values():
        if any(
            target[symbol] > (1.0 / 3.0) + 1e-12
            for symbol in RISK_SYMBOLS
        ):
            raise ValidationError("candidate non-cash target exceeds one third.")
        if any(
            not math.isclose(value * 3.0, round(value * 3.0), abs_tol=1e-12)
            for symbol, value in target.items()
            if symbol in CANDIDATE_SYMBOLS
        ):
            raise ValidationError("candidate sleeves are not exact thirds.")
...
def _evaluate(
    ...
    comparator_ids = (ABLATION_ID, STATIC_ID, SHY_ID, SPY_ID, PARENT_ID)
    comparators = {
        strategy_id: {
            cost_id: _window_metrics(value) for cost_id, value in costs.items()
        }
        for strategy_id, costs in series.items()
        if strategy_id in comparator_ids
    }
    candidate_costs = {
        cost_id: _window_metrics(value)
        for cost_id, value in series[CANDIDATE_ID].items()
    }
    ...
    for cost_id, rate in _COSTS.items():
        ...
        composites[cost_id] = {
            ...
            "comparison_to_parent": {
                window: _compare(
                    metrics[window],
                    comparators[PARENT_ID][cost_id][window],
                )
    ...
    target_window_metrics: dict[str, object] = {}
    for window, start, end in _WINDOWS:
        ...
        ablation_window = {
            item: target
            for item, target in actions[ABLATION_ID].items()
            if start <= item <= end
        }
        static_window = {
            item: target
        ...
        target_window_metrics[window] = {
            "action_count": len(candidate_window),
            "max_non_cash_target_weight": _number(
                max(
                    target[symbol]
            ...
            "divergent_decisions_from_return_only_ablation": _divergence(
                candidate_window, ablation_window
            ),
    ...
    candidate = {
        ...
        "decision_cost_comparisons": {
            comparator_id: {
                window: _compare(
                    ...
                    comparators[comparator_id]["decision"][window],
                )
                for window, _, _ in _WINDOWS
            }
            for comparator_id in (ABLATION_ID, STATIC_ID, SPY_ID)
        },
        "portfolio_composite": {
            "construction": (
                "80pct_actual_monthly_spy_ief_60_40_plus_"
                "20pct_actual_candidate_targets"
            ),
        ...
        "target_contract": {
            ...
            "window_metrics": target_window_metrics,
            "max_non_cash_target_weight": _number(
                max(
                    target[symbol]
                    for target in actions[CANDIDATE_ID].values()
            ...
            "divergent_decisions_from_return_only_ablation": _divergence(
                actions[CANDIDATE_ID], actions[ABLATION_ID]
            ),
            "divergent_decisions_from_static": _divergence(
    ...
    integrity = {
        ...
        "first_oos_initial_transition_turnover_is_one": math.isclose(
            series[CANDIDATE_ID]["decision"].turnover[0],
            1.0,
            abs_tol=1e-12,
        ),
        "weight_sums_and_nonnegative_constraints_verified": True,
        "four_month_return_and_sample_volatility_verified": True,
        "daily_pairwise_pearson_average_rank_verified": True,
        "strict_negative_return_sleeve_transfers_to_shy": True,
...
def _decisions(
    ...
    *,
    ...
    contributions = {
        symbol: _float(value)
        for symbol, value in oos["constituent_contributions"].items()
    }
    ...
    common = {
        "full_annualized_return_positive": _float(
            oos["annualized_return"]
        )
        > 0.0,
        "full_sharpe_at_least_0_60": _required_float(
            oos["sharpe_ratio"]
        )
        >= 0.60,
        "full_max_drawdown_at_most_0_30": _float(
            oos["max_drawdown"]
        )
        <= 0.30,
        ...
        "stress_annualized_return_positive": _float(
            stress["oos"]["annualized_return"]
        )
        > 0.0,
        "stress_sharpe_at_least_0_50": _required_float(
            stress["oos"]["sharpe_ratio"]
        )
        >= 0.50,
        "max_fold_positive_log_return_share_at_most_0_70": (
            bool(fold_shares) and max(fold_shares) <= 0.70
        ),
        "all_non_cash_assets_held": all(
            symbol in oos["symbols_held"] for symbol in RISK_SYMBOLS
        ),
        "at_least_three_non_cash_assets_contribute_positively": sum(
            contributions[symbol] > 0.0 for symbol in RISK_SYMBOLS
        )
        >= 3,
        "max_non_cash_target_at_most_one_third": _float(
            item["target_contract"]["max_non_cash_target_weight"]
        )
        <= (1.0 / 3.0) + 1e-12,
    ...
    static_gate = {
        "sharpe_exceeds_static_by_0_05": _required_float(
            static["oos"]["sharpe_ratio_delta"]
        )
        >= 0.05,
        "drawdown_no_more_than_0_02_worse": _float(
            static["oos"]["max_drawdown_delta"]
        )
        <= 0.02,
        "at_least_two_fold_sharpe_wins": sum(
            _required_float(
    ...
    ablation = item["decision_cost_comparisons"][ABLATION_ID]
    ablation_drawdown = _float(
        comparators[ABLATION_ID]["decision"]["oos"]["max_drawdown"]
    )
    ablation_value_paths = {
        "sharpe_improves_by_at_least_0_03": _required_float(
            ablation["oos"]["sharpe_ratio_delta"]
        )
        >= 0.03,
        "drawdown_improves_by_at_least_5pct_relative": _float(
            ablation["oos"]["max_drawdown_improvement"]
        )
        >= 0.05 * ablation_drawdown,
        "annualized_return_drag_at_most_0_01": _float(
            ablation["oos"]["annualized_return_delta"]
        )
        >= -0.01,
    }
    ablation_gate = {
        "at_least_12_divergent_monthly_decisions": (
            item["target_contract"][
                "divergent_decisions_from_return_only_ablation"
            ]
            >= 12
        ),
        "ravc_factors_add_value": (
    ...
    defensive = {
        ...
        "sharpe_exceeds_spy_by_0_10": _required_float(
            spy_comparison["sharpe_ratio_delta"]
        )
        >= 0.10,
    ...
    full_improvement = (
        _float(composite["decision"]["metrics"]["oos"]["compounded_log_return"])
        - _float(
    ...
    portfolio_gate = {
        "full_sharpe_improves_by_0_02": _required_float(
            decision_composite["oos"]["sharpe_ratio_delta"]
        )
        >= 0.02,
        "annualized_return_reduction_at_most_0_0075": _float(
        ...
        "positive_sharpe_improvement_in_two_folds": sum(
            _required_float(
        ...
        >= 2,
        "fold_3_sharpe_improvement_nonnegative": _required_float(
            decision_composite["oos_fold_3"]["sharpe_ratio_delta"]
        )
        >= 0.0,
        "stress_full_sharpe_improvement_nonnegative": _required_float(
...
def _artifact_manifest(result: Mapping[str, object]) -> dict[str, object]:
    return {
        "record_type": "keller_flexible_asset_allocation_artifact_manifest",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "inputs": {
            "protocol_sha256": _PROTOCOL_HASH,
...
def _summary(result: Mapping[str, object]) -> str:
    ...
    lines = [
        "# V5.87 Keller flexible asset allocation tournament result",
        "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_keller_flexible_asset_allocation_tournament(args.output_root)
    except (OSError, ValidationError, ValueError) as exc:
        print(f"keller_flexible_asset_allocation_tournament_status=blocked:{exc}")
        return 2
    print("keller_flexible_asset_allocation_tournament_status=completed")
