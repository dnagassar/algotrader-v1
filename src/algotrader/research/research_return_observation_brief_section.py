"""Metadata-only advisory section for research return observation brief items."""
...
_SECTION_TYPE = "research_return_observation_brief_section"
...
class ResearchReturnObservationBriefSection:
    """Primitive advisory metadata grouping return observation brief items."""
    ...
    title: str
    summary: str
    items: tuple[ResearchReturnObservationBriefItem, ...]
    ...
    def to_dict(self) -> dict[str, object]:
        """Return deterministic primitive-only return observation section metadata."""
...
def build_research_return_observation_brief_section(
    section_id: str,
    title: str,
    summary: str,
    items: Iterable[ResearchReturnObservationBriefItem],
) -> ResearchReturnObservationBriefSection:
    """Build a deterministic advisory-only return observation brief section."""

    checked_items = _items_tuple(items)
    return ResearchReturnObservationBriefSection(
...
def _items_tuple(
    values: Iterable[ResearchReturnObservationBriefItem],
) -> tuple[ResearchReturnObservationBriefItem, ...]:
    try:
        items = tuple(values)
    except TypeError as exc:
        raise ValidationError(
            "items must be an iterable of ResearchReturnObservationBriefItem."
        ) from exc
    ...
    for index, item in enumerate(items):
        if type(item) is not ResearchReturnObservationBriefItem:
            raise ValidationError(
                f"items[{index}] must be a ResearchReturnObservationBriefItem."
            )
...
def _validate_fixed_metadata(
    ...
    if section_type != _SECTION_TYPE:
        raise ValidationError(
            "section_type must be exactly "
            "research_return_observation_brief_section."
        )
    if status != _STATUS:
