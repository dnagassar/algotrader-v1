...
Phase 320 / Milestone 320 - Crypto Paper Order-List Gap Diagnostics keeps the
M319 state taxonomy and adds local deterministic detail around the M318
`recent_orders=[]` observation. Snapshot JSON/JSONL output now records whether
the recent-order query was attempted and available, its returned count, and the
currently unspecified limit/status/asset/symbol/time-window filters. Recent
order observations preserve safe `order_id` and `client_order_id` fields when
the local fake/SDK boundary exposes them. The revalidation brief now reports
target receipt correlation fields, whether the target receipt matched a recent
order, the match basis (`client_order_id`, `broker_order_id`,
`symbol_side_notional`, or `none`), and a deterministic `order_list_gap_reason`
such as `recent_order_query_returned_empty`,
`target_order_not_in_recent_order_results`,
`receipt_missing_correlation_id`, `order_query_unavailable`, or
`insufficient_order_query_metadata`. The M318 shape remains classified as
`receipt_and_position_observed_with_order_list_gap`; the local reason is
`recent_order_query_returned_empty` because the accepted receipt and BTCUSD
position were observed while the post-submit order-list query returned zero
orders. This milestone is read-only/local diagnostics only and adds no submit
...
deterministic fake/local-only contract for paper recent-order observations
before any future paper close or exit probe. The internal
`AlpacaRecentOrderQuery` contract is versioned as
...
`alpaca_sdk_client.get_orders`. The SDK boundary is the only place that
converts this internal query to alpaca-py's `GetOrdersRequest`; injected fake
clients receive the internal query object directly. Snapshot JSON/JSONL/text
and revalidation output now expose the complete query contract fields plus
`recent_order_query_metadata_complete` and
`recent_order_query_metadata_missing_fields`. Old M318/M320-shaped logs still
classify as `receipt_and_position_observed_with_order_list_gap` with
`order_list_gap_reason=recent_order_query_returned_empty`, but their query
metadata is explicitly incomplete, so the local evidence proves only what the
snapshot layer requested or recorded and does not prove external broker order
state. This milestone adds no submit command, close/sell order, Alpaca call,
network use, live profile/live URL behavior, credential access, broker write
...
adds a deterministic local-only `explicit_close_probe_prompt_review` section to
`paper-lab-revalidation-brief` JSON and text output. The section is
operator-review-only and reports `manual_review_only=true`,
`broker_action_performed=false`, `close_order_submitted=false`,
`prompt_ready_for_operator_review`, whether it was generated from M327
`future_close_probe_preparation`, the observed M327 readiness flag, observed
M326 eligibility status, observed BTCUSD symbol/quantity, recent-order query
metadata completeness, required final fresh pre-submit snapshot, required final
operator confirmation, future probe scope, blocking reasons, a recommended
operator action, review-only prompt text, and a non-submit future command
template. The prompt review becomes ready only when M327 is ready, M326 reports
`eligible_for_explicit_operator_approval`, close-preview evidence exists, the
fresh snapshot checklist is complete, recent-order query metadata is complete,
and the `credentials_redacted` marker is present. Eligible evidence recommends
`review_explicit_paper_close_probe_prompt_and_decide_whether_to_authorize_separate_m329`;
blocked evidence recommends
`complete_future_close_probe_preparation_before_prompt_review`. This milestone
adds no submit command, broker-side close/sell/cancel/liquidation behavior,
Alpaca call, network use, live profile/live URL support, credential access,
broker write path, scheduler, autonomous loop, research-layer authorization,
portfolio mutation, profit inference, or unsafe pytest behavior.

Phase 329A / Milestone 329A - Missing BTCUSD Close-Preview Evidence Repair
wires the existing local-only `paper-close-preview` command to optionally append
a durable `paper_close_preview_designed` JSONL event via `--output-run-log` and
`--output-run-id`. The command still reads local run-log evidence only, does not
load runtime config, does not build a broker, and has no submit flag. The event
records the preview-only BTCUSD sell shape, including `quantity`,
`max_quantity`, `observed_position_quantity`,
...
Phase 337 / Milestone 337 - ETF/SMA Offline Backtest Summary v1 adds the first
deterministic measurement artifact for the M335/M336 ETF/SMA path. The new
`etf_sma_offline_backtest_summary` contract accepts caller-supplied
deterministic local or synthetic bars, an immutable config, and an explicit
`as_of` date. It ignores future bars, computes SMA signals only from closes
available at or before each signal bar, applies exposure with a one-bar delay to
avoid same-bar lookahead, and reports summary metrics including bar counts,
signal/exposure/defensive counts, posture changes, strategy total return,
buy-and-hold benchmark total return, max drawdown, and latest posture.

The summary preserves `research_only`, `paper_lab_candidate`,
`not_live_authorized`, and `profit_claim=none`, while eligibility remains
`research_measurement_only`. Its limitations are explicitly
`synthetic_or_local_input_only`, `zero_cost_or_declared_cost_model`,
`no_slippage_model_unless_explicitly_added`, `no_live_or_paper_authorization`,
and `not_profit_evidence`. M337 adds no broker action, no broker preview or
staging, no `ExecutionIntent`, no `ExecutionPlan`, no credentials, no network,
no market-data fetch, no live profile/live URL, no scheduler/autonomous
behavior, and no order/fill/account/portfolio mutation. The next step is local
...
Milestone 355 adds `paper-lab-spy-close-submit`, the explicit paper-only SPY
cleanup close command. It consumes the ready M354 artifact, performs fresh
read-only account, position, and open/all/closed SPY order observations, blocks
duplicate `paper-order-close-m355_spy_paper_close_submit` evidence, and submits
exactly one SPY equity sell `market`/`day` quantity order only when all gates
pass. The M355 paper run wrote
`runs/paper_lab/m355_spy_paper_close_submit.jsonl`, submitted exactly once with
quantity `0.032905647`, and received broker status `accepted` for order
`56a2f690-f4ad-4572-bcf4-1a479398fe55`. The immediate post-submit read-only
observation found the SPY position still present at `0.032905647`, one open
SPY sell order for the M355 client order id, no fill from the submit response,
and final state `close_submit_accepted_pending_reconciliation`. M355 does not
authorize retry, cancel, replace, liquidate, close-position endpoint usage,
additional orders, live trading, or autonomous follow-up. The only recommended
next milestone is M356: read-only cleanup/revalidation of the pending close
order and SPY position state.

Milestone 363 records the operator review for the stale accepted M355 SPY paper
close order. It is documentation and local artifact review only: no broker
mutation, no paper-profile command, no direct SDK query, no Alpaca UI corrective
action, and no cancel, replace, retry, close-position, liquidation, second sell,
or buy. The review treats M362 as the latest valid diagnostic and uses the
local evidence chain in M358, M359, M361, and M362; M360 is explicitly excluded
as settlement evidence because it failed the profile gate.

M363 evidence: M355 submitted the paper-only SPY sell close order
`paper-order-close-m355_spy_paper_close_submit`; M358, M359, M361, and M362 all
observed the same open accepted unfilled SPY sell order matched by both client
order id and broker order id `56a2f690-f4ad-4572-bcf4-1a479398fe55`; the SPY
position remains present at `0.032905647`; order status remains `accepted` /
`OrderStatus.ACCEPTED`; order quantity remains `0.032905647`; filled quantity
remains `0`; `filled_at` remains blank; and M362 reported no unavailable
observations or blockers. The submitted timestamp is
`2026-06-01T21:49:57.297904+00:00`, with the M362 age estimate about 11.9 hours.

The M363 inference is that the M355 close order is stale enough to require an
operator decision before any corrective action. The system must not
autonomously cancel, replace, retry, liquidate, submit another close order, use
a close-position endpoint, or otherwise mutate broker/account/portfolio state.
Any corrective action must be a separate explicitly approved paper-only
milestone.

M363 records the available operator options: Option A keep waiting and run
another read-only check later; Option B prepare a separate cancel-only
preview/review milestone with no immediate cancel in M363; Option C prepare a
separate cancel-and-replace design/preview milestone with no immediate cancel or
...
progression until the stale-order cause is understood. The recommended option
is Option B as the next safest executable path: a separate paper-only
cancel-readiness/preview milestone for the stale accepted close order. That next
milestone still must not cancel unless it is explicitly scoped as a
cancel-submit milestone with hard operator confirmation.

The hard gate for any future corrective action remains separate milestone
approval. Before any future cancel-submit, replace, retry, close-position, or
liquidation milestone, the operator must first run a fresh read-only snapshot
and verify paper profile ready, SPY position still present, exactly one matching
open stale M355 sell order, no other conflicting open SPY orders, no filled or
