...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P30-BL-001 | Simple scalar threshold indicator definition | mechanical indicator definitions | Normalized source package: [`phase31_p30_bl_001_source_package.md`](phase31_p30_bl_001_source_package.md); Tier A review: [`phase31_p30_bl_001_tier_a_review.md`](phase31_p30_bl_001_tier_a_review.md); evidence gap routing plan: [`phase31_p30_bl_001_evidence_gap_routing_plan.md`](phase31_p30_bl_001_evidence_gap_routing_plan.md); mechanics-only summary: [`phase31_p30_bl_001_mechanics_only_review_summary.md`](phase31_p30_bl_001_mechanics_only_review_summary.md); final disposition: [`phase31_p30_bl_001_final_disposition.md`](phase31_p30_bl_001_final_disposition.md); Phase 32 routing: [`phase32_dataset_specific_validation_candidate_selection.md`](phase32_dataset_specific_validation_candidate_selection.md) | future threshold-style advisory evaluator | scalar threshold advisory state | `indicator_value`, comparator, threshold | `Decimal` for `indicator_value`; explicit comparator and threshold metadata | Directly relevant to `indicator_value >= threshold` mechanics | not applicable for mechanical sources; no dataset-specific validation | mechanical transformation only / methodology support only | Tier A conditionally supports mechanics and methodology only; Step 6 records mechanics-only conditional support; Step 7 dispositioned mechanics-only support only; Phase 32 routes next work toward dataset-specific evidence; no validated artifact, no validated signal definition, no production threshold rationale, no dataset validation, no implementation approval | P0 | mechanics-only dispositioned | P30-BL-001 is closed only in the mechanics-only sense; next work should source dataset-specific threshold or validation evidence under `P30-BL-002` only if a concrete package can be found, otherwise source a better P0 replacement |
| P30-BL-002 | Threshold sanity check for `indicator_value` | threshold sanity-check studies | Phase 32 selected this id as the current sourcing handle only if a dataset-specific evidence package can be produced; sourcing plan: [`phase32_p30_bl_002_source_package_sourcing_plan.md`](phase32_p30_bl_002_source_package_sourcing_plan.md); revised source package: [`phase32_p30_bl_002_source_package.md`](phase32_p30_bl_002_source_package.md), with 23 candidate-only scout-normalized entries; primary-source verification gate: [`phase32_p30_bl_002_primary_source_verification_gate.md`](phase32_p30_bl_002_primary_source_verification_gate.md); limited formal review intake plan: [`phase32_p30_bl_002_limited_formal_review_intake_plan.md`](phase32_p30_bl_002_limited_formal_review_intake_plan.md); S01 formal review: [`phase32_p30_bl_002_s01_formal_review.md`](phase32_p30_bl_002_s01_formal_review.md); S03 formal review: [`phase32_p30_bl_002_s03_formal_review.md`](phase32_p30_bl_002_s03_formal_review.md); S08 formal review: [`phase32_p30_bl_002_s08_formal_review.md`](phase32_p30_bl_002_s08_formal_review.md); S05 formal review: [`phase32_p30_bl_002_s05_formal_review.md`](phase32_p30_bl_002_s05_formal_review.md) | future threshold-style advisory evaluator | scalar threshold advisory state | `indicator_value`, threshold, comparator, `as_of` | `Decimal`; UTC-aware timestamp metadata if applicable | Directly relevant to non-arbitrary threshold selection only after future validation; S01 contributes negative-control/no-lookahead guardrail context only; S03 contributes data-snooping/OOS guardrail context only; S08 contributes methodology-only PIT/no-lookahead/survivorship/restatement constraints only; S05 contributes limited candidate-evidence planning only | partial; Step 4 verifies selected source identity/intake eligibility only, Step 5 plans review criteria only, Step 6 reviews S01 only for limited negative-control/no-lookahead use, Step 7 reviews S03 only for limited negative-control/data-snooping/OOS guardrail use, Step 8 reviews S08 only for methodology-only PIT use, and Step 9 reviews S05 only for limited candidate-evidence planning | threshold sanity check / S01 and S03 negative-control reviews plus S08 methodology-only review plus S05 limited candidate-evidence planning | No reviewed threshold, no comparator evidence accepted, no validation window accepted, no project-local deterministic reproduction accepted, no reproducibility notes accepted for deterministic implementation, no robustness or out-of-sample evidence accepted for implementation; Phase 32 Step 9 does not validate or approve the candidate and does not make S05 implementation-ready | P0 | needs review | Plan a docs-only structured local reproduction/evidence checklist for S05 under the S08 PIT/no-lookahead/survivorship/restatement constraints and S01/S03 guardrails; do not use any completed review as threshold approval, signal validation, artifact promotion, or implementation readiness |
...
sourcing as the next research direction. `P30-BL-002` is the current sourcing
target only if a concrete evidence package can be produced; a better P0
replacement should be sourced first if it offers stronger traceable
dataset-specific evidence. This is routing status only.

Phase 32 Step 2 adds the `P30-BL-002` source-package sourcing plan. It defines
the minimum package fields, acceptable and unacceptable source material,
minimum review-readiness criteria, and rejection/replacement triggers before
formal review can begin. It does not collect sources, validate `P30-BL-002`,
approve a threshold, or make any candidate implementation-ready.

Phase 32 Step 3 adds the revised `P30-BL-002` source-package collection and
normalization pass. It normalizes 23 candidate-only entries from the supplied
Claude, Perplexity, and Gemini/browser scout reports, deduplicates overlapping
material, classifies preliminary routing categories, and records source-level
...
It defines review order, shared criteria, source-specific criteria, pass/fail
outcomes, required review artifacts, non-claims, and blockers for a later
formal review. It does not formally review, approve, validate, promote, or
implement `P30-BL-002`.
...
That pass can support falsification and timing-bias guardrail design only. It
does not validate a signal, approve a threshold, create a validated research
artifact, create a validated signal definition, or make `P30-BL-002`
implementation-ready. The next route is `P30-BL-002-S03` formal review.
...
negative-control/data-snooping/OOS guardrail use. That pass can support
...
signal, approve a threshold, create a validated research artifact, create a
validated signal definition, or make `P30-BL-002` implementation-ready. The
next route is `P30-BL-002-S08` formal review to lock point-in-time methodology
before candidate-evidence review.
...
pass can support point-in-time methodology framing, survivorship-bias
...
not validate a signal, approve a threshold, create a validated research
artifact, create a validated signal definition, or make `P30-BL-002`
implementation-ready. At Step 8, the next route was `P30-BL-002-S05` formal
review as the first limited candidate-evidence source under the S08
PIT/no-lookahead, survivorship, and restatement expectations.
...
candidate-evidence planning. That pass can support a bounded time-series
...
candidate signal-definition discussion only. It does not validate a signal,
approve a threshold, create a validated research artifact, create a validated
signal definition, or make `P30-BL-002` implementation-ready.
...
1. Future step: structured local reproduction/evidence planning for S05 using
   the Phase 32 Step 9 formal review, with additional sourcing or a better P0
   replacement if unresolved source gaps block useful planning, docs-only.
