...
| P30-BL-002 | Threshold sanity check for `indicator_value` | threshold sanity-check studies | unsourced; Phase 32 selected this id as the current sourcing handle only if a dataset-specific evidence package can be produced | future threshold-style advisory evaluator | scalar threshold advisory state | `indicator_value`, threshold, comparator, `as_of` | `Decimal`; UTC-aware timestamp metadata if applicable | Directly relevant to non-arbitrary threshold selection | unknown; must be made explicit before review | threshold sanity check | No reviewed threshold, no comparator evidence, no validation window, no point-in-time assumptions, no reproducibility notes, no robustness or out-of-sample evidence | P0 | sourcing target | Collect a normalized source package with dataset scope, point-in-time input assumptions, threshold or parameter rationale, no-lookahead controls, reproducibility notes, robustness or out-of-sample evidence, limitations, and non-claims; if this cannot be sourced, replace with a better P0 candidate before formal review |
...
1. Future step: source-package collection for `P30-BL-002` only if it can
   provide dataset-specific threshold or validation evidence; otherwise source
   a better P0 replacement before formal review, docs-only.
