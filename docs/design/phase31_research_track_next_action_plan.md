...
The review passes `P30-BL-002-S01` only for limited
negative-control/no-lookahead use and routes the next review to
`P30-BL-002-S03`. It does not validate a signal, approve a threshold, create a
...
The review conditionally passes `P30-BL-002-S05` only for limited
candidate-evidence planning. It may support a bounded time-series momentum
...
signal-definition discussion. It does not validate a signal, approve a
...
It groups candidate public/easy data source feasibility, future ETF universe
requirements, and future benchmark/cash proxy requirements. It does not
...
normalization in
[`phase33_broad_etf_primary_evidence_text_intake_normalization.md`](phase33_broad_etf_primary_evidence_text_intake_normalization.md).
It records the second Perplexity source-gathering report as external scout
material, normalizes reported primary-text availability for high-priority
sources, marks unresolved identity and citation-quality issues, and recommends
pausing until primary text and citation verification is complete. It does not
...
- Phase 32 Step 8 records the S08-only formal review. `S08` passes only for
  ...
  readiness.
- Phase 32 Step 9 records the S05-only formal review. `S05` conditionally
...
- Phase 33 Step 3 records the broad-ETF grouped data feasibility, universe,
  and benchmark boundary. It compares Stooq, Yahoo Finance / yfinance, Nasdaq
  Data Link where applicable, Alpha Vantage free/retail APIs, official ETF
  issuer pages, FRED where applicable, and broker historical data as
  candidate/context source categories only; defines future ETF universe and
...
- Phase 33 Step 12 records the broad-ETF evidence source collection
  ...
  available.
- Phase 33 Step 13 records the broad-ETF primary evidence text intake
  normalization. It treats the second Perplexity source-gathering report as
...
10. Phase 32 Step 5: `P30-BL-002` limited formal review intake plan.
   ...
   source.
11. Phase 32 Step 6: `P30-BL-002-S01` formal review.
   This step is complete. It passes S01 only for limited
   negative-control/no-lookahead use, records unresolved exact timing,
...
14. Phase 32 Step 9: `P30-BL-002-S05` formal review.
   This step is complete. It conditionally passes S05 only for limited
   candidate-evidence planning, records unresolved project-local reproduction,
   PIT/no-lookahead audit, roll/cost, OOS, multiple-testing, and implementation
...
20. Phase 32 Step 15: S05 primary-verification questionnaire and outreach
   ...
   implementation.
21. Phase 32 Step 16: S05 public documentation verification sweep.
   This step is complete. It separates primary documentation, secondary
   documentation, and inference for public-documentation signals; assigns
   cautious feasibility labels for AQR, CSI, Pinnacle, Norgate, Portara,
   TradeStation, institutional feeds, broker APIs, and public/proxy sources;
...
23. Phase 32 Step 18: S05 non-exact proxy reproduction boundary.
   This step is complete. It defines proxy reproduction as a controlled
   approximation for methodology mechanics and research workflow testing only,
   distinguishes exact, partial, proxy, and methodology-only/context routes,
   and keeps proxy work from supporting S05 validation, implementation,
   production thresholds, or trading readiness.
24. Phase 32 Step 19: S05 proxy route selection boundary.
   This step is complete. It compares modern futures subset, reduced-universe
   futures, ETF/index proxy, AQR factor-level context, manually reconstructed
   published-table check, and pause/defer routes. It keeps multiple proxy
...
28. Phase 33 Step 2: broad-ETF moving-average source package.
   ...
   implementation, or trading implication.
29. Phase 33 Step 3: broad-ETF data feasibility, universe, and benchmark
   boundary.
   This step is complete. It groups candidate source feasibility, future ETF
   universe requirements, and future benchmark/cash proxy requirements for the
...
41. Phase 33 Step 12: broad-ETF evidence source collection normalization.
   ...
   proxy, reproduction, validation, implementation, evaluator behavior, signal
   computation, or trading implication.
42. Phase 33 Step 13: broad-ETF primary evidence text intake normalization.
   This step is complete. It normalizes reported primary-text availability
   from a second Perplexity source-gathering report, records citation-quality
   cautions, marks Faber and `ETF-ACADEMIC-001` as conditional only after
   primary text and citation verification, keeps "Simple Market Timing with
   Moving Averages" unresolved, and tracks related Zakamulin/SSRN-style
   papers as separate candidates only if exact citations are verified. It does
...
49. Phase 33 Step 20: broad-ETF return-construction boundary.
   This step is complete. It compares raw close returns, adjusted close
   returns, explicit total-return construction, vendor-provided total-return
   series, cash/T-bill return series, and a zero-return placeholder as
   unresolved options only. It records required decisions for dividends,
   distributions, splits, corporate actions, adjusted-data transparency,
   expenses, cash-rate conversion, benchmark comparability, compounding,
   inception alignment, missing/stale data, frequency alignment, and timing.
   It keeps return construction blocked for approval and recommends a
   docs-only no-lookahead/as-of protocol boundary next. It does not approve a
